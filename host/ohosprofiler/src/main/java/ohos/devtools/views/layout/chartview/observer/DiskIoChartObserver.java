...
import ohos.devtools.services.diskio.DiskIoDao;
 ...
 * @since 2021/10/25
 */
public class DiskIoChartObserver implements IChartEventObserver {
    private static final Logger LOGGER = LogManager.getLogger(DiskIoChartObserver.class);
     ...
     */
    public DiskIoChartObserver(ProfilerChart chart, long sessionId, boolean chartFold) {
        this.chart = chart;
    ...
    public void refreshView(ChartDataRange range, long firstTimestamp, boolean useCache) {
        ...
        if (chartStop || !useCache) {
            queryResult = DiskIoDao.getInstance().getData(sessionId, start, end, firstTimestamp, true);
        } else {
            queryResult = DiskIoDataCache.getInstance().getData(sessionId, start, end);
        }
