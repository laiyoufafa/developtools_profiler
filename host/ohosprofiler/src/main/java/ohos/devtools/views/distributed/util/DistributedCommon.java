...
public class DistributedCommon {
    ...
    public static long x2ns(final int coordX, Rectangle rect) {
        return x2ns(coordX, rect, DistributedTracePanel.getDURATION());
    }
    ...
    public static double ns2x(long ns, Rectangle rect) {
        return ns2x(ns, rect, DistributedTracePanel.getDURATION());
    }
    ...
    public static void drawStringMiddleHeight(Graphics2D g2, String str, final Rectangle rect) {
        ...
        if (chartNum >= str.length()) {
            ...
            if (ProfilerLogManager.isDebugEnabled()) {
                LOGGER.debug("drawStringMiddleHeight not in display area");
            }
        }
    }
}
