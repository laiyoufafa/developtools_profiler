    ...
    namespace SmartPerf {
        ...
        void FPS::setPackageName(std::string pkgName)
        {
            ...
            if (sps.size() > 0) {
                ...
                char cmd[100];
                sprintf(cmd, "hidumper -s 10 |grep surface |grep %s", cur_layer.c_str());
        ...
        FpsInfo FPS::getFpsInfo(int is_video, int is_camera)
        {
            ...
            {
                layerName = std::string("EntryView");
            } else {
                std::vector<std::string> sps;
                GPUtils::mSplit(this->pkg_name, ".", sps);
                layerName = std::string(sps[2].c_str() + std::string("0"));
            }
        ...
        FpsInfo FPS::GetSurfaceFrameDataGB(std::string name)
        {
            ...
            if (cnt < 5) {
                ...
                fpsInfo.dumpInfoLess5Cnt++;
                if (fpsInfo.dumpInfoLess5Cnt >= 2) {
                }
                fpsInfo.fps = fpsInfo.pre_fps;

                return fpsInfo;
            }
            fpsInfo.dumpInfoLess5Cnt = 0;
