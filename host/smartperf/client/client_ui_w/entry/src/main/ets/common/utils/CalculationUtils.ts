...
export default class CalculationUtils {
    ...
    calculateCaton(fpsJitterList: Array<string>): number{
        console.log("CalculationUtils FpsJitters" + JSON.stringify(fpsJitterList));
        let num = 0
        for (let i = 0; i < fpsJitterList.length; i++) {
            let jitter = fpsJitterList[i].split(",");
            console.log("CalculationUtils jitter" + i + JSON.stringify(jitter));
            for(let j = 0; j < jitter.length; j++) {
                console.log("CalculationUtils jitterValue" + j + JSON.stringify(jitter[j]));
                var n = Number(jitter[j]);
                console.log("CalculationUtils jitterValue" + j + "n:" + n);
                if (!isNaN(n) && (n / 1e6 > 100)) {
                    num++
                }
            }
        }
        console.log("CalculationUtils num" + num);
        return num
    }
