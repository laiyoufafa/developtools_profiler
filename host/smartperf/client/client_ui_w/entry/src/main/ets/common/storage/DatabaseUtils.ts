...
import { TIndexInfo, GPData, TGenneralInfo } from '../data/DatabaseEntity';
import { sql_t_index_info, dbPath, dbVersion } from '../storage/ConstantsDb'
...
export default {
    ...
    async createTable(pathSuffix: number): Promise<void> {
        ...
        console.log(TAG + "--> createTable t_index_info 1  ")
        console.log(TAG + "--> createTable t_index_info 2  STORE_CONFIG_Index.name:" + STORE_CONFIG_Index.name)
        console.log(TAG + "--> createTable t_index_info 2  STORE_CONFIG_Index:" + JSON.stringify(STORE_CONFIG_Index))
        console.log(TAG + "--> createTable t_index_info 2 dbVersion:" + dbVersion)
        dataRdb.getRdbStore(globalThis.abilityContext, STORE_CONFIG_Index, dbVersion)
            .then(rdbStore => {
                console.log(TAG + "--> createTable t_index_info 3  getRdbStore")
                rdbStore.executeSql(sql_t_index_info, null).catch(err => {
                    console.log(TAG + "--> createTable t_index_info err:" + err)
                })
                console.log(TAG + "--> createTable start execute sql_t_index_info:" + sql_t_index_info)
                return rdbStore
            })
            .then(rdbStore => {
                console.log(TAG + "--> createTable  dbPath: " + dbPath + "/" + pathSuffix.toString())
            })
    },

    //插入表( T_GENERAL_INFO)
    insertGeneraData(tableName: string, tGenneralInfo: TGenneralInfo, sessionId: String) {
        var strMap = new Map;
        for (let k of Object.keys(tGenneralInfo)) {
            strMap.set(k, tGenneralInfo[k]);
            console.log(TAG + "--> TransData..." + k + ":" + tGenneralInfo[k])
        }
        const valueInsert = {
        ...
        console.log(TAG + "--> insert into insertGeneraData 0000:")
        const STORE_CONFIG = {
            name: "gp.db"
        }
        let predicates = new dataRdb.RdbPredicates('t_general_info')
        predicates.equalTo("sessionId", sessionId.toString())
        dataRdb.getRdbStore(globalThis.abilityContext, STORE_CONFIG, dbVersion, (err, rdbStore) => {
            console.log(TAG + "--> insert into  insertGeneraData :tableName:" + tableName + "| valueInsert:" + JSON.stringify(valueInsert))
            rdbStore.insert(tableName, valueInsert).catch(err => {
                console.log(TAG + "--> insert into  insertGeneraData err:" + err)
                rdbStore.update(valueInsert, predicates).catch(err => {
                    console.log(TAG + "--> update into  insertGeneraData err:" + err)
                }).then(res => {
                    console.log(TAG + "--> update into  insertGeneraData success:")
                })
            })
        })
    },
    ...
    insertData(tableName: string, pathSuffix: number, tindexInfo: TIndexInfo) {
        ...
        for (let k of Object.keys(tindexInfo)) {
            ...
            console.log(TAG + "--> TransData..." + k + ":" + tindexInfo[k])
        }
        const valueInsert = {
        ...
        //const STORE_CONFIG  = { name: dbName, path: dbPath+"/"+pathSuffix.toString()}
        const STORE_CONFIG = {
            name: pathSuffix + ".db"
        }
        dataRdb.getRdbStore(globalThis.abilityContext, STORE_CONFIG, dbVersion, (err, rdbStore) => {
    ...
    //查询表( T_GENERAL_INFO)
    async queryGeneralData(dbPath: string): Promise<Array<TGenneralInfo>> {
        const STORE_CONFIG = {
            name: "gp.db"
        }
        let results = Array<TGenneralInfo>()
        try {
            console.log("resultSet queryGeneralData:0000000000000")
            return await dataRdb.getRdbStore(globalThis.abilityContext, STORE_CONFIG, 1)
                .then(rdbStore => {
                    console.log("resultSet queryGeneralData:1111111111111")
                    let predicates = new dataRdb.RdbPredicates('t_general_info')
                ...
                .then(resultSet => {
                    console.log("resultSet queryGeneralData:222222222222222222:" + resultSet.rowCount)
                    while (resultSet.goToNextRow()) {
                        ...
                        tGenneralInfo.board = board
                        tGenneralInfo.deviceTypeName = deviceTypeName
                        tGenneralInfo.brand = brand
                        tGenneralInfo.version = version
                        tGenneralInfo.sn = sn
                        results.push(tGenneralInfo)
                    }
                    console.log("resultSet queryGeneralData:333333333333333333:" + results.length)
                    return results
                })

        } catch (err) {
            console.log("resultSet queryGeneralData:err" + err)
        }
    },
    ...
    async queryData(dbPath: string): Promise<Array<TIndexInfo>> {
        ...
        try {
            ...
            console.log("resultSet queryData:err" + err)
        }
    },
    ...
    gpArray2Index(gpDatas: Array<GPData>): TIndexInfo{
        console.log(TAG + "--> gpArray2Index...called")
        ...
        console.log(TAG + "--> gpArray2Index...called finished")
        return tIndexInfo
    }
}
