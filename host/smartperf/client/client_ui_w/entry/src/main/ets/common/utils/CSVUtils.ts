 ...
 */
import { TGenneralInfo } from '../data/DatabaseEntity';
import { TIndexInfo } from '../data/DatabaseEntity';

//垂直生成 TGenneralInfo to string
export function csvGeneralInfo(tGenneralInfo: TGenneralInfo): string {
    let data = ""
    var strMap = new Map;
    for (let k of Object.keys(tGenneralInfo)) {
        data += k + "," + tGenneralInfo[k] + "\n"
        console.log("--> CSV TransData..." + k + ":" + tGenneralInfo[k])
    }
    return data
}
...
export function csvTIndexInfo(tIndexInfos: Array<TIndexInfo>): string {
    ...
    console.log("csvTIndexInfo:tittle:" + tittle);
    let data = ""
    ...
    //data头包含undefined异常值  需要去除
    let headStr = data.substring(0, data.indexOf(","))
    console.log("csvTIndexInfo:tittle data:" + data);
    console.log("csvTIndexInfo:tittle headStr data:" + headStr);

    let result = tittle + data
    console.log("csvTIndexInfo:tittle result:" + result);
    return result
}
