...
public:
    GpuCounter() = default;
    ~GpuCounter() = default;
    void AppendNewData(uint64_t ts, int counterId, double value);
    const std::deque<uint64_t>& TimeStamp() const;
    const std::deque<int>& CounterId() const;
    const std::deque<double>& Value() const;

private:
    std::deque<uint64_t> ts_ = {};
    std::deque<int> counterId_ = {};
    std::deque<double> value_ = {};
};
...
public:
    SliceData() = default;
    ~SliceData() = default;
    void AppendNewData(int sliceId, uint64_t startTs, uint64_t endTs, std::string start_time, std::string end_time, double value);
    const std::deque<int>& SliceId() const;
    const std::deque<uint64_t>& TimeStamp() const;
    const std::deque<uint64_t>& EndTs() const;
    const std::deque<double>& Value() const;
    const std::deque<std::string>& StartTime() const;
    const std::deque<std::string>& EndTime() const;
...
private:
    ...
    std::deque<double> value_ = {};
    std::deque<std::string> starttime_ = {};
    std::deque<std::string> endtime_ = {};
};
