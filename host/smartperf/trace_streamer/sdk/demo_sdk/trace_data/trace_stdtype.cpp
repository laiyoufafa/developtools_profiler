...
void GpuCounter::AppendNewData(uint64_t ts, int counterId, int value)
{
    ts_.emplace_back(ts);
...
const std::deque<int>& GpuCounter::Value() const
{
    return value_;
}
...
void SliceData::AppendNewData(int sliceId, uint64_t startTs, uint64_t endTs, int value)
{
...
const std::deque<int>& SliceData::Value() const
{
    return value_;
}
