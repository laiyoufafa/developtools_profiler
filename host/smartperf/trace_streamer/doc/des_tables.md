...
* hisys_event_measure ： 记录了HiSysEvent事件相关数据，目前HiSysEvent事件包括了：异常事件，IDE事件，器件状态事件。
...
* sysevent_measure_table ： 记录所有的system event事件的相关数据，及其相关表的映射信息。
