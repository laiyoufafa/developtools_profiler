...
namespace {
enum Index { ID = 0, FILE_ID, SERIAL_ID, SYMBOL, PATH };
}
...
bool PerfFilesTable::CanFilterId(const char op, size_t& rowCount)
{
    switch (op) {
...
int PerfFilesTable::Cursor::Filter(const FilterConstraints& fc, sqlite3_value** argv)
{
    // reset indexMap_
...
int PerfFilesTable::Cursor::Column(int column) const
{
    switch (column) {
        case ID:
