...
namespace {
enum Index { FILE_ID = 0, ID, FUNCTION_INFO_INDEX, COUNT, SIZE, PARENT_ID };
}
...
int JsHeapTraceNodeTable::Cursor::Column(int col) const
{
    switch (col) {
        case FILE_ID:
            ...
            break;
        case ID:
            sqlite3_result_int64(context_, static_cast<int64_t>(jsHeapTraceNode_.IDs()[CurrentRow()]));
            break;
