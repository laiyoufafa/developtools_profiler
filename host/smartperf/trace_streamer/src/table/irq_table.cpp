...
void IrqTable::CreateCursor()
{
    cursor_ = std::make_unique<Cursor>(dataCache_);
}

IrqTable::Cursor::Cursor(const TraceDataCache* dataCache)
    : TableBase::Cursor(dataCache, 0, static_cast<uint32_t>(dataCache->GetConstIrqData().Size())),
