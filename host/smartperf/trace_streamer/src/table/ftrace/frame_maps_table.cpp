...
enum Index { ID = 0, SRC_ROW, DST_ROW };
...
void FrameMapsTable::EstimateFilterCost(FilterConstraints& fc, EstimatedIndexInfo& ei)
{
...
void FrameMapsTable::FilterByConstraint(FilterConstraints& fc, double& filterCost, size_t rowCount)
{
...
bool FrameMapsTable::CanFilterId(const char op, size_t& rowCount)
{
    switch (op) {
...
int FrameMapsTable::Cursor::Filter(const FilterConstraints& fc, sqlite3_value** argv)
{
    // reset indexMap_
    ...
    for (size_t i = 0; i < cs.size(); i++) {
        ...
        switch (c.col) {
            ...
            case SRC_ROW:
                ...
                break;
            case DST_ROW:
                indexMap_->MixRange(c.op, static_cast<uint64_t>(sqlite3_value_int(argv[i])), frameMapsObj_.DstIndexs());
                break;
            default:
                break;
        }
...
int FrameMapsTable::Cursor::Column(int column) const
{
    switch (column) {
        case ID:
