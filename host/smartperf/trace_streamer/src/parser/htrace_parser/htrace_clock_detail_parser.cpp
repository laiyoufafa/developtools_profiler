...
void HtraceClockDetailParser::Parse(const base::ProfilerTraceFileHeader* profilerTraceFileHeader) const
{
    if (streamFilters_->clockFilter_->HasInitSnapShot()) {
        TS_LOGW("already has clock snapshot!!!");
        return;
    ...
    if (0 != profilerTraceFileHeader->data.realtime) {
    ...
    if (0 != profilerTraceFileHeader->data.realtimeCoarse) {
    ...
    if (0 != profilerTraceFileHeader->data.monotonic) {
    ...
    if (0 != profilerTraceFileHeader->data.monotonicCoarse) {
    ...
    if (0 != profilerTraceFileHeader->data.monotonicRaw) {
