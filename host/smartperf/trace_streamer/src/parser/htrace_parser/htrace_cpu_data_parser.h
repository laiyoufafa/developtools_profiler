...
public:
    ...
    ~HtraceCpuDataParser();
    void Parse(CpuData& tracePacket, uint64_t ts);
    void Finish();
    ...
    public:
        ...
        {
            ts_ = ts;
            cpuUsageData_ = std::make_unique<CpuUsageInfo>(usage);
            cpuDataType_ = TSCpuDataType_Usage;
        }
        void SetThreadInfo(uint64_t ts, ThreadInfo& threadInfo)
        {
            ts_ = ts;
            threadDataData_ = std::make_unique<ThreadInfo>(threadInfo);
        ...
        uint64_t ts_;
        std::unique_ptr<CpuUsageInfo> cpuUsageData_;
        std::unique_ptr<ThreadInfo> threadDataData_;
...
private:
    std::string threadStateDesc_[THREAD_WAITING + 1] = {"undefined", "Running", "Sleep", "Sloped", "Watting"};
