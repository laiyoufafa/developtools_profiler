...
std::unique_ptr<PerfFileReader> PerfFileReader::Instance(const std::string &fileName, size_t begin)
{
    ...
    std::unique_ptr<PerfFileReader> reader = std::make_unique<PerfFileReader>(fileName, fp, begin);
    ...
    : fp_(fp), fileName_(fileName), fileBegin_(begin)
{
    ...
    if (fp != nullptr) {
        if (fstat(fileno(fp), &fileStat) != -1 and fileStat.st_size > 0) {
            fileSize_ = fileStat.st_size - fileBegin_;
        }
    }

    if (fseek(fp_, fileBegin_, SEEK_SET) != 0) {
        HLOGE("fseek() failed");
    }
}
...
bool PerfFileReader::ReadAttrSection()
{
    ...
    if (!SeekFromBegin(header_.attrs.offset)) {
        return false;
    }
...
bool PerfFileReader::ReadIdsForAttr(const perf_file_attr &attr, std::vector<uint64_t> *ids)
{
    if (attr.ids.size > 0) {
        ...
        if (!SeekFromBegin(attr.ids.offset)) {
            return false;
        }
