...
private:
    ...
    //ReaderType type_ = ReaderType::FILE;
    size_t fileSize_ = 0;
    size_t fileBegin_ = 0;
    ...
    bool SeekFromBegin(size_t offset)
    {
        if (isMemory_) {
            if (offset > buffSize_) {
                HLOGE("seek offset(%zu) >= buffer size(%zu)", offset, buffSize_);
                return false;
            }
            buffCurrent_ = offset;
        } else {
            if (fseek(fp_, fileBegin_ + offset, SEEK_SET) != 0) {
                HLOGE("fseek(%zu) failed", offset);
                return false;
            }
        }
