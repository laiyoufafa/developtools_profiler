        ...
        switch (mode) {
            case ChartMode.Byte:
                node.frame!.width = Math.floor(
                    ((node.drawSize || node.size) / total) * canvas_frame.width
                );
                break;
            case ChartMode.Count:
                node.frame!.width = Math.floor(
                    ((node.drawCount || node.count) / total) *
                        canvas_frame.width
                );
                break;
            case ChartMode.Duration:
                node.frame!.width = Math.floor(
                    ((node.drawDur || node.dur) / total) * canvas_frame.width
                );
                break;
            default:
