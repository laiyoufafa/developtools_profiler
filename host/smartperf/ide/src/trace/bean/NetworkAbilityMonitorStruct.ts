...
export class NetworkAbilityMonitorStruct extends BaseStruct {
    ...
    static draw(context2D: any, data: NetworkAbilityMonitorStruct) {
        if (data.frame) {
            ...
            if (data.startNS === NetworkAbilityMonitorStruct.hoverNetworkAbilityStruct?.startNS) {
                ...
                let drawHeight: number = Math.floor(((data.value || 0) * (data.frame.height || 0) * 1.0) / NetworkAbilityMonitorStruct.maxNetworkRate);
                ...
                context2D.globalAlpha = 0.6;
                context2D.lineWidth = 1;
                let drawHeight: number = Math.floor(((data.value || 0) * (data.frame.height || 0)) / NetworkAbilityMonitorStruct.maxNetworkRate);
                context2D.fillRect(data.frame.x, data.frame.y + data.frame.height - drawHeight, width, drawHeight)
            }
        }
        context2D.globalAlpha = 1.0;
