...
export class ProcedureLogicWorkerSchedulingAnalysis extends LogicHandler {
    ...
    cpuFreqMap: Map<number, Array<CpuMeasure>> = new Map<
        number,
        Array<CpuMeasure>
    >();
    cpuIdle0Map: Map<number, Array<CpuMeasure>> = new Map<
        number,
        Array<CpuMeasure>
    >();
            ...
            switch (data.type) {
                ...
                case 'scheduling-CPU Frequency Thread':
                    if (data.params.list) {
                        self.postMessage({
                            id: data.id,
                            action: data.action,
                            results: this.handlerFreqThreadData(
                                convertJSON(data.params.list) || []
                            ),
                        });
                    } else {
                ...
                case 'scheduling-CPU Irq':
                    if (this.cpuAnalysisMap.has('irq')) {
                        ...
                        if (data.params.list) {
                            let res = this.groupIrgDataByCpu(
                                convertJSON(data.params.list) || []
                            );
                ...
                case 'scheduling-Thread CpuUsage':
                    if (data.params.list) {
                        self.postMessage({
                            id: data.id,
                            action: data.action,
                            results: this.handlerThreadCpuUsageData(
                                convertJSON(data.params.list) || []
                            ),
                        });
                    } else {
                ...
                case 'scheduling-Thread RunTime':
                    if (data.params.list) {
                        ...
                        self.postMessage({
                            ...
                            action: data.action,
                            results: arr.map((it) => {
                                it.maxDurationStr = getProbablyTime(
                                    it.maxDuration
                                );
                                it.pName =
                                    this.processMap.get(it.pid) || 'null';
                ...
                case 'scheduling-Thread Freq':
                    if (data.params.list) {
                        self.postMessage({
                            id: data.id,
                            action: data.action,
                            results: this.handlerThreadFreqData(
                                convertJSON(data.params.list) || []
                            ),
                        });
                    } else {
...
where cpu not null
  ...
  order by cpu,ts;`;
    ...
    groupIrgDataByCpu(arr: Irq[]) {
        ...
        let target: Map<number, CpuAnalysis[]> = new Map<
            number,
            CpuAnalysis[]
        >();
    ...
    getEffectiveFrequencyDur(m: CpuMeasure) {
        ...
        for (let idle of filterArr) {
            dur +=
                Math.min(m.ts + m.dur, idle.ts + idle.dur) -
                Math.max(m.ts, idle.ts);
        }
        m.dur = dur;
    }

    groupFreqByCpu(arr: CpuMeasure[]) {
        let map: Map<number, Array<CpuMeasure>> = new Map<
            number,
            Array<CpuMeasure>
        >();
        for (let i = 0, len = arr.length; i < len; i++) {
        ...
        for (let key of map.keys()) {
            let obj = map.get(key)!.reduce((group: any, ca) => {
                ...
                if (group[value]) {
                    ...
                    group[value].min =
                        group[value].min < ca.dur ? group[value].min : ca.dur;
                    group[value].max =
                        group[value].max > ca.dur ? group[value].max : ca.dur;
                    group[value].count = group[value].count + 1;
                    group[value].avg = (
                        group[value].sum / group[value].count
                    ).toFixed(2);
                } else {
                    group[value] = {
                        cpu: ca.cpu,
    ...
    handlerFreqThreadData(arr: FreqThread[]) {
        let cpuFreqArr: CpuMeasure[] = (
            this.cpuFreqMap.get(this.cpu) || []
        ).filter((it) => it.value === this.freq);
        let map: Map<
            number,
            {
                tid: number;
        ...
        cpuFreqArr.map((it) => {
            ...
            for (let tf of threads) {
                ...
                if (map.has(tf.tid)) {
                    ...
                    map.get(tf.tid)!.durStr = getProbablyTime(
                        map.get(tf.tid)!.dur
                    );
                } else {
                    map.set(tf.tid, {
    ...
    handlerThreadCpuUsageData(arr: Array<ThreadCpuUsage>) {
        ...
        for (let obj of source) {
            obj['bigPercent'] =
                sumBig === 0 ? '0' : ((obj.big / sumBig) * 100).toFixed(2);
            obj['midPercent'] =
                sumMid === 0 ? '0' : ((obj.mid / sumMid) * 100).toFixed(2);
            obj['smallPercent'] =
                sumSmall === 0
                    ? '0'
                    : ((obj.small / sumSmall) * 100).toFixed(2);
            obj['bigTimeStr'] = getProbablyTime(obj.big);
            obj['midTimeStr'] = getProbablyTime(obj.mid);
            obj['smallTimeStr'] = getProbablyTime(obj.small);
        }
        let map: Map<string, Array<ThreadCpuUsage>> = new Map<
            string,
            Array<ThreadCpuUsage>
        >();
    ...
    handlerThreadFreqData(
        ...
        let obj: any = arr.reduce((group: any, tf) => {
            for (let fa of tf.freqArr) {
                ...
                if (group[`${cpu}-${freq}`]) {
                    group[`${cpu}-${freq}`].time =
                        group[`${cpu}-${freq}`].time + fa.dur;
                    group[`${cpu}-${freq}`].timeStr = getProbablyTime(
                        group[`${cpu}-${freq}`].time
                    );
                    group[`${cpu}-${freq}`].ratio = (
                        (group[`${cpu}-${freq}`].time / sumDur) *
                        100
                    ).toFixed(2);
                } else {
                    group[`${cpu}-${freq}`] = {
