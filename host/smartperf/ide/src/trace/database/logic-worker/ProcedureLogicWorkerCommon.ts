...
class MerageBeanDataSplit {
    ...
    recursionChargeInitTree(
        ...
        if (
            ...
            (splitMapData[symbolName] = splitMapData[symbolName] || []).push(
                node
            );
            node.isStore++;
        }
    ...
    recursionChargeByRule(
        ...
        if (node.initChildren.length > 0) {
            node.initChildren.forEach((child) => {
                if (rule(child)) {
                    (splitMapData[ruleName] =
                        splitMapData[ruleName] || []).push(child);
                    child.isStore++;
                }
    ...
    pruneChildren(splitMapData: any, node: MerageBean, symbolName: string) {
        if (node.initChildren.length > 0) {
            node.initChildren.forEach((child) => {
                child.isStore++;
                (splitMapData[symbolName] =
                    splitMapData[symbolName] || []).push(child);
    ...
    findSearchNode(data: MerageBean[], search: string, parentSearch: boolean) {
        data.forEach((node) => {
            if (
                (node.symbolName != undefined &&
                    node.symbolName.includes(search)) ||
                parentSearch
            ) {
