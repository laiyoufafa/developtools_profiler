...
export class ProcedureLogicWorkerPerf extends LogicHandler {
    ...
    handle(data: any): void {
        ...
        if (data && data.type) {
            switch (data.type) {
                ...
                case 'perf-action':
                    if (data.params) {
                        let filter = data.params.filter(
                            (item: any) =>
                                item.funcName == 'getCurrentDataFromDb'
                        );
                        if (filter.length == 0) {
                            // @ts-ignore
                            self.postMessage({
    ...
    getCurrentDataFromDb(selectionParam: any) {
        ...
        let processes = selectionParam.perfAll
            ? []
            : selectionParam.perfProcess;
        let threads = selectionParam.perfAll ? [] : selectionParam.perfThread;
        let sql = '';
        if (cpus.length != 0 || processes.length != 0 || threads.length != 0) {
            let arg1 =
                cpus.length > 0 ? `or s.cpu_id in (${cpus.join(',')}) ` : '';
            let arg2 =
                processes.length > 0
                    ? `or thread.process_id in (${processes.join(',')}) `
                    : '';
            let arg3 =
                threads.length > 0
                    ? `or s.thread_id in (${threads.join(',')})`
                    : '';
...
group by callchain_id,s.thread_id,thread_state,process_id) p`,
    ...
    initCallChainBottomUp(callChains: PerfCallChain[]) {
        callChains.forEach((callChain, index) => {
            ...
            if (
                index + 1 < callChains.length &&
                callChains[index + 1].sampleId == callChain.sampleId
            ) {
                PerfCallChain.setPreviousNode(callChain, callChains[index + 1]);
            }
            if (
                callChains.length == index + 1 ||
                callChains[index + 1].sampleId != callChain.sampleId
            ) {
    ...
    initCallChainTopDown(callChains: PerfCallChain[]) {
        ...
        callChains.forEach((callChain, index) => {
            ...
            if (callChainDatum.length > 1) {
                PerfCallChain.setNextNode(
                    callChainDatum[callChainDatum.length - 2],
                    callChainDatum[callChainDatum.length - 1]
                );
            }
        });
    }
    ...
    setCallChainName(callChain: PerfCallChain) {
        ...
        if (callChain.symbolId == -1) {
            if (
                this.filesData[callChain.fileId] &&
                this.filesData[callChain.fileId].length > 0
            ) {
                callChain.fileName =
                    this.filesData[callChain.fileId][0].fileName;
            ...
            if (
                this.filesData[callChain.fileId] &&
                this.filesData[callChain.fileId].length > callChain.symbolId
            ) {
                callChain.fileName =
                    this.filesData[callChain.fileId][
                        callChain.symbolId
                ...
                callChain.path =
                    this.filesData[callChain.fileId][callChain.symbolId].path;
            } else {
                callChain.fileName = 'unkown';
            }
        }
    ...
    addProcessThreadStateData(callChain: PerfCallChain) {
        ...
        threadCallChain.name =
            this.threadData[callChain.tid].threadName ||
            'Thead' + '(' + callChain.tid + ')';
        ...
        threadStateCallChain.fileName =
            threadStateCallChain.name == '-' ? 'Unkown Thead State' : '';
    ...
    addGroupData(callChain: PerfCallChain) {
        this.callChainData[callChain.sampleId] =
            this.callChainData[callChain.sampleId] || [];
    ...
    addOtherCallchainData(countSample: PerfCountSample, list: any[]) {
        ...
        threadCallChain.canCharge = false;
        threadCallChain.name =
            this.threadData[countSample.tid].threadName ||
            'Thead' + '(' + countSample.tid + ')';
        let threadStateCallChain = new PerfCallChain(); //新增的线程状态数据
    ...
    freshCurrentCallchains(samples: PerfCountSample[], isTopDown: boolean) {
        ...
        samples.forEach((sample) => {
            ...
            if (callChains.length > 0) {
                let root =
                    this.currentTreeMapData[
                        callChains[topIndex].name + sample.pid
                    ];
                if (root == undefined) {
        ...
        Object.values(this.currentTreeMapData).forEach((merageData: any) => {
            if (rootMerageMap[merageData.pid] == undefined) {
                ...
                processMerageData.symbolName =
                    this.threadData[merageData.tid].processName ||
                    `Process(${merageData.pid})`;
    ...
    merageChildrenByIndex(
        ...
        let node;
        if (
            currentNode.initChildren.filter(
                (child: PerfCallChainMerageData) => {
    ...
    groupNewTreeNoId(sampleIds: string[], isTopDown: boolean): any[] {
        ...
        for (let i = 0; i < sampleIds.length; i++) {
            ...
            if (callChains.length > 0) {
                let root =
                    this.currentTreeMapData[
                        callChains[topIndex].name + callChains[topIndex].pid
                    ];
                if (root == undefined) {
                    root = new PerfCallChainMerageData();
                    this.currentTreeMapData[
                ...
                PerfCallChainMerageData.merageCallChain(
                    root,
                    callChains[topIndex],
                    isTopDown
                );
    ...
    recursionChargeInitTree(
        node: PerfCallChainMerageData,
        symbolName: string,
        isSymbol: boolean
    ) {
        if (
            (isSymbol && node.symbolName == symbolName) ||
            (!isSymbol && node.libName == symbolName)
        ) {
            (this.splitMapData[symbolName] =
                this.splitMapData[symbolName] || []).push(node);
            node.isStore++;
        }
    ...
    recursionChargeByRule(
        ...
        if (node.initChildren.length > 0) {
            node.initChildren.forEach((child) => {
                if (rule(child)) {
                    (this.splitMapData[ruleName] =
                        this.splitMapData[ruleName] || []).push(child);
                    child.isStore++;
                }
    ...
    pruneChildren(node: PerfCallChainMerageData, symbolName: string) {
        if (node.initChildren.length > 0) {
            node.initChildren.forEach((child) => {
                child.isStore++;
                (this.splitMapData[symbolName] =
                    this.splitMapData[symbolName] || []).push(child);
            ...
            if (item.parentNode != undefined) {
                if (item.isStore == 0 && item.searchShow) {
                    ...
                    while (
                        ...
                        !(parentNode.isStore == 0 && parentNode.searchShow)
                    ) {
    ...
    findSearchNode(
        ...
        data.forEach((node) => {
            if ((node.symbol && node.symbol.includes(search)) || parentSearch) {
                ...
                node.isSearch =
                    node.symbol != undefined && node.symbol.includes(search);
    ...
    splitAllProcess(list: any[]) {
        list.forEach((item: any) => {
            this.allProcess.forEach((process) => {
                if (item.select == '0') {
                    this.recursionChargeInitTree(
                        process,
                        item.name,
                        item.type == 'symbol'
                    );
                } else {
                    this.recursionPruneInitTree(
                        process,
                        item.name,
                        item.type == 'symbol'
                    );
...
export class PerfCallChain {
    ...
    static setPreviousNode(
        currentNode: PerfCallChain,
        prevNode: PerfCallChain
    ) {
        currentNode.previousNode = prevNode;
        prevNode.nextNode = currentNode;
    }

    static merageCallChain(
        currentNode: PerfCallChain,
...
export class PerfCallChainMerageData extends ChartStruct {
    ...
    set total(data: number) {
        this.#total = data;
        this.weight = `${timeMsFormat2p(
            this.dur * (ProcedureLogicWorkerPerf.cmdLineResult?.fValue || 1)
        )}`;
    ...
    static merageCallChain(
        ...
        if (currentNode.symbolName == '') {
            currentNode.symbol = `${callChain.name}  ${
                callChain.fileName ? `(${callChain.fileName})` : ''
            }`;
        ...
        if (callChain[isTopDown ? 'nextNode' : 'previousNode'] == undefined) {
            ...
            currentNode.self = timeMsFormat2p(
                currentNode.selfDur *
                    (ProcedureLogicWorkerPerf.cmdLineResult?.fValue || 1)
            );
        }
