...
export class ProcedureLogicWorkerNativeMemory extends LogicHandler {
    ...
    HEAP_FRAME_MAP: Map<number, Array<HeapTreeDataBean>> = new Map<
        number,
        Array<HeapTreeDataBean>
    >();
        ...
        if (data && data.type) {
            switch (data.type) {
                ...
                case 'native-memory-queryDataDICT':
                    ...
                    dict.map((d: any) =>
                        this.DATA_DICT.set(d['id'], d['data'])
                    );
                ...
                case 'native-memory-chart-action':
                    if (data.params) {
                        postMessage(
                            data.id,
                            data.action,
                            this.resolvingActionNativeMemoryChartData(
                                data.params
                            )
                        );
                    }
                ...
                case 'native-memory-queryNativeHookStatistic':
                    if (data.params.list) {
                        postMessage(
                            data.id,
                            data.action,
                            this.handleNativeHookStatisticData(
                                convertJSON(data.params.list)
                            )
                        );
                    } else {
        ...
        list.forEach((item) => {
            if (item.lastLibId == null) {
                ...
                this.responseTypes.push({
                    key: item.lastLibId,
                    value:
                        this.groupCutFilePath(item.lastLibId, item.value) ||
                        '-',
                });
            }
        });
    ...
    ): Array<NativeHookCallInfo | NativeMemory | HeapStruct> {
    ...
    resolvingActionNativeMemoryChartData(
        paramMap: Map<string, any>
    ): Array<HeapStruct> {
        let nativeMemoryType: number = paramMap.get(
            'nativeMemoryType'
        ) as number;
        ...
        if (nativeMemoryType == 1) {
            let temp = this.NATIVE_MEMORY_DATA.filter(
                (ne) => ne.eventType == 0 || ne.eventType == 2
            );
            ...
            let temp = this.NATIVE_MEMORY_DATA.filter(
                (ne) => ne.eventType == 1 || ne.eventType == 3
            );
            filterLen = temp.length;
            filterLevel = this.getFilterLevel(filterLen);
            temp.map((ne) => putArr(ne, filterLevel));
        ...
        if (
            this.chartComplete.has(0) &&
            this.chartComplete.has(1) &&
            this.chartComplete.has(2)
        ) {
            this.NATIVE_MEMORY_DATA = [];
        }
        return arr;
    ...
    resolvingActionNativeMemoryStack(paramMap: Map<string, any>) {
        ...
        frameArr.map((frame) => {
            ...
            target.symbol =
                this.groupCutFilePath(
                    frame.symbolId,
                    this.DATA_DICT.get(frame.symbolId) || ''
                ) ?? '';
            target.library =
                this.groupCutFilePath(
                    frame.fileId,
                    this.DATA_DICT.get(frame.fileId) || ''
                ) ?? '';
            target.title = `[ ${target.symbol} ]  ${target.library}`;
            target.type =
                target.library.endsWith('.so.1') ||
                target.library.endsWith('.dll') ||
                target.library.endsWith('.so')
                    ? 0
                    : 1;
            arr.push(target);
        });
    ...
    resolvingActionNativeMemory(
        ...
        let filter = dataSource.filter((item) => {
            ...
            let filterNative = this.getTypeFromIndex(
                parseInt(filterEventType),
                item,
                statisticsSelection
            );
            let filterLastLib =
                filterResponseType == -1
                    ? true
                    : filterResponseType == item.lastLibId;
        ...
        for (let i = 0, len = filter.length; i < len; i++) {
            ...
            memory.timestamp =
                this.realTimeDif == 0
                    ? getTimeString(hook.startTs)
                    : formatRealDateMs(hook.startTs + this.realTimeDif);
            memory.state =
                hook.endTs > leftNs && hook.endTs <= rightNs
                    ? 'Freed'
                    : 'Existing';
            ...
            if (frame != null && frame != undefined) {
                memory.symbol = this.groupCutFilePath(
                    frame.symbolId,
                    this.DATA_DICT.get(frame.symbolId) || ''
                );
                memory.library = this.groupCutFilePath(
                    frame.fileId,
                    this.DATA_DICT.get(frame.fileId) || 'Unknown Path'
                );
            } else {
                memory.symbol = '-';
                memory.library = '-';
    ...
    mergeTree(target: NativeHookCallInfo, src: NativeHookCallInfo) {
        ...
        src.heapPercent = `${((src!.size / this.selectTotalSize) * 100).toFixed(
            1
        )}%`;
        if (len == 0) {
            src.children.push(target);
        } else {
            let index = src.children.findIndex(
                (hook) =>
                    hook.symbol == target.symbol && hook.depth == target.depth
            );
            if (index != -1) {
                let srcChild = <NativeHookCallInfo>src.children[index];
                srcChild.count += target.count;
                srcChild!.countValue = `${srcChild.count}`;
                srcChild!.countPercent = `${(
                    (srcChild!.count / this.selectTotalCount) *
                    100
                ).toFixed(1)}%`;
                if (target.children.length > 0) {
                    this.mergeTree(
                        <NativeHookCallInfo>target.children[0],
                        <NativeHookCallInfo>srcChild
                    );
                } else {
                    srcChild.size += target.size;
                    srcChild.heapSizeStr = `${getByteWithUnit(src!.size)}`;
                    srcChild.heapPercent = `${(
                        (srcChild!.size / this.selectTotalSize) *
                        100
                    ).toFixed(1)}%`;
                }
            } else {
    ...
    traverseSampleTree(stack: NativeHookCallInfo, hook: NativeHookStatistics) {
        ...
        stack.countPercent = `${(
            (stack.count / this.selectTotalCount) *
            100
        ).toFixed(1)}%`;
        ...
        stack.heapPercent = `${(
            (stack.size / this.selectTotalSize) *
            100
        ).toFixed(1)}%`;
    ...
    getTypeFromIndex(
        ...
        if (indexOf < 3) {
            ...
            if (
                selectionElement.memoryTap != undefined &&
                selectionElement.max != undefined
            ) {
                if (selectionElement.memoryTap.indexOf('Malloc') != -1) {
                    return (
                        item.eventType == 'AllocEvent' &&
                        item.heapSize == selectionElement.max
                    );
                } else if (selectionElement.memoryTap.indexOf('Mmap') != -1) {
                    return (
                        item.eventType == 'MmapEvent' &&
                        item.heapSize == selectionElement.max
                    );
                } else {
                    return (
                        item.subType == selectionElement.memoryTap &&
                        item.heapSize == selectionElement.max
                    );
                }
            }
    ...
    queryStatisticCallchainsSamples(
        leftNs: number,
        rightNs: number,
        types: Array<number>
    ) {
    ...
    freshCurrentCallchains(
        ...
        samples.forEach((sample) => {
            ...
            if (callChains.length > 0) {
                ...
                NativeHookCallInfo.merageCallChainSample(
                    root,
                    callChains[topIndex],
                    sample
                );
                if (callChains.length > 1) {
                    this.merageChildrenByIndex(
                        root,
                        callChains,
                        topIndex,
                        sample,
        ...
        threads.forEach((merageData: any) => {
            if (rootMerageMap[merageData.tid] == undefined) {
                ...
                threadMerageData.symbolName = `${
                    merageData.threadName || 'Thread'
                } [${merageData.tid}]`;
    ...
    groupCallchainSample(paramMap: Map<string, any>) {
        ...
        filter.forEach((sample) => {
            let currentNode =
                groupMap[sample.tid + '-' + sample.eventId] ||
                new NativeHookStatistics();
    ...
    merageChildrenByIndex(
        ...
        let isEnd = isTopDown
            ? callChainDataList.length == index + 1
            : index == 0;
        let node;
        if (
    ...
    setMerageName(currentNode: NativeHookCallInfo) {
        currentNode.symbol =
            this.groupCutFilePath(
                currentNode.symbolId,
                this.DATA_DICT.get(currentNode.symbolId) || ''
            ) ?? 'unkown';
                ...
                if (item.funcName && item.funcArgs) {
                    switch (item.funcName) {
                        case 'groupCallchainSample':
                            this.groupCallchainSample(
                                item.funcArgs[0] as Map<string, any>
                            );
                            break;
                        case 'getCallChainsBySampleIds':
                            this.freshCurrentCallchains(
                                this.currentSamples,
                                item.funcArgs[0]
                            );
                            break;
                        case 'hideSystemLibrary':
                            merageBeanDataSplit.hideSystemLibrary(
                                this.allThreads,
                                this.splitMapData
                            );
                            break;
                        ...
                        case 'splitAllProcess':
                            merageBeanDataSplit.splitAllProcess(
                                this.allThreads,
                                this.splitMapData,
                                item.funcArgs[0]
                            );
                        ...
                        case 'resetAllNode':
                            ...
                            break;
                        case 'resotreAllNode':
                            merageBeanDataSplit.resotreAllNode(
                                this.splitMapData,
                                item.funcArgs[0]
                            );
                            break;
