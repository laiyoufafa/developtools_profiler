...
export class ProcedureLogicWorkerFileSystem extends LogicHandler {
        ...
        if (data && data.type) {
            switch (data.type) {
                case 'fileSystem-init':
                    ProcedureLogicWorkerFileSystem.data_dict =
                        data.params as Map<number, string>;
                    this.initCallchains();
                    break;
                ...
                case 'fileSystem-action':
                    if (data.params) {
                        let filter = data.params.args.filter(
                            (item: any) =>
                ...
                case 'fileSystem-queryStack':
                    let res = this.getStacksByCallchainId(
                        data.params.callchainId
                    );
                    self.postMessage({
                        id: data.id,
                        action: data.action,
                ...
                case 'fileSystem-queryFileSysEvents':
                    if (data.params.list) {
                        ...
                        postMessage(
                            data.id,
                            data.action,
                            this.supplementFileSysEvents(res, this.tab)
                        );
                    } else {
                        this.tab = data.params.tab;
                        this.queryFileSysEvents(
                            data.params.leftNs,
                            data.params.rightNs,
                            data.params.typeArr,
                            data.params.tab
                ...
                case 'fileSystem-queryIOEvents':
                    if (data.params.list) {
                        ...
                        postMessage(
                            data.id,
                            data.action,
                            this.supplementIoEvents(res)
                        );
                    } else {
                        this.queryIOEvents(
                            data.params.leftNs,
                            data.params.rightNs,
                            data.params.diskIOipids
                        );
                    }
                    break;
            }
    ...
    getStacksByCallchainId(id: number) {
        ...
        for (let s of stacks) {
            ...
            st.path = (
                ProcedureLogicWorkerFileSystem.data_dict.get(s.pathId) ??
                'Unknown Path'
            )
                .split('/')
                .reverse()[0];
            st.symbol = `${
                s.symbolsId == null
                    ? s.ip
                    : ProcedureLogicWorkerFileSystem.data_dict.get(
                          s.symbolsId
                      ) ?? ''
            } (${st.path})`;
            st.type =
                st.path.endsWith('.so.1') ||
                st.path.endsWith('.dll') ||
                st.path.endsWith('.so')
                    ? 0
                    : 1;
            arr.push(st);
    ...
    supplementIoEvents(res: Array<IoCompletionTimes>) {
        return res.map((event) => {
            ...
            event.durPer4kStr =
                event.durPer4k == 0 ? '-' : getProbablyTime(event.durPer4k);
            event.sizeStr = getByteWithUnit(event.size);
            event.durStr = getProbablyTime(event.dur);
            event.path = event.pathId
                ? ProcedureLogicWorkerFileSystem.data_dict.get(event.pathId) ??
                  '-'
                : '-';
            // @ts-ignore
            event.operation = DISKIO_TYPE_MAP[`${event.type}`] || 'UNKNOW';
            let stacks =
                ProcedureLogicWorkerFileSystem.callChainsMap.get(
                    event.callchainId
                ) || [];
            if (stacks.length > 0) {
                let stack = stacks[0];
                event.backtrace = [
                    stack.symbolsId == null
                        ? stack.ip
                        : ProcedureLogicWorkerFileSystem.data_dict.get(
                              stack.symbolsId
                          ) ?? '',
    ...
    supplementFileSysEvents(res: Array<FileSysEvent>, tab: string) {
        res.map((r) => {
            let stacks = ProcedureLogicWorkerFileSystem.callChainsMap.get(
                r.callchainId
            );
            ...
            if (tab == 'events') {
                ...
                r.error = r.error ?? '0x0';
                r.path =
                    ProcedureLogicWorkerFileSystem.data_dict.get(r.fileId) ??
                    '-';
            }
            // @ts-ignore
    ...
    initCallchains() {
        ...
        this.handlerMap.set(
            'fileSystem',
            new FileSystemCallTreeHandler('fileSystem', this.queryData)
        );
        this.handlerMap.set(
            'io',
            new FileSystemCallTreeHandler('io', this.queryData)
        );
        this.handlerMap.set(
            'virtualMemory',
    ...
    initCallChainTopDown(list: any[]) {
        list.forEach((callchain: FileCallChain) => {
            if (
                ProcedureLogicWorkerFileSystem.callChainsMap.has(
                    callchain.callChainId
                )
            ) {
                ProcedureLogicWorkerFileSystem.callChainsMap
                    .get(callchain.callChainId)!
                    .push(callchain);
            } else {
                ProcedureLogicWorkerFileSystem.callChainsMap.set(
                    callchain.callChainId,
                    [callchain]
                );
            }
...
class FileSystemCallTreeHandler {
    ...
    queryFileSamples(selectionParam: any) {
        let sql = '';
        if (
            selectionParam.fileSystemType != undefined &&
...
where s.end_ts between $startTime + t.start_ts and $endTime + t.start_ts ${sql} and callchain_id != -1;`,
    ...
    queryIOSamples(selectionParam: any) {
        ...
        if (selectionParam.diskIOReadIds.length > 0) {
            sql += `or (s.ipid in (${selectionParam.diskIOReadIds.join(
                ','
            )}) and s.type in (1,3)) `;
        }
        if (selectionParam.diskIOWriteIds.length > 0) {
            sql += `or (s.ipid in (${selectionParam.diskIOWriteIds.join(
                ','
            )}) and s.type in (2,4)) `;
        }
        this.queryData(
    ...
    freshCurrentCallchains(samples: FileSample[], isTopDown: boolean) {
        ...
        samples.forEach((sample) => {
            ...
            if (callChains.length > 1) {
                let root =
                    this.currentTreeMapData[
                        callChains[topIndex].symbolsId +
                            '' +
                            callChains[topIndex].pathId +
                            sample.pid
                    ];
                if (root == undefined) {
            ...
            currentNode.initChildren.filter((child: any) => {
                if (
                    ...
                    (child.symbolsId != null &&
                        child.symbolsId ==
                            callChainDataList[index]?.symbolsId &&
    ...
    static merageCallChainSample(
        ...
        if (currentNode.processName == '') {
            ...
            currentNode.processName =
                sample.processName || `Process(${sample.pid})`;
        }
        if (isEnd) {
