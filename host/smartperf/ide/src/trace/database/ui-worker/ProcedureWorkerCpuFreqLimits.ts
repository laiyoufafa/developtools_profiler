        ...
        if (req.canvas) {
            ...
            if (req.isHover) {
                ...
                CpuFreqLimitsStruct.hoverCpuFreqLimitsStruct =
                    req.params.hoverCpuFreqLimitsStruct;
            }
            CpuFreqLimitsStruct.selectCpuFreqLimitsStruct =
                req.params.selectCpuFreqLimitsStruct;
            for (let re of filter) {
                CpuFreqLimitsStruct.draw(req.context, re, maxFreq);
            }
...
export function freqLimits(
    ...
    if (use && res.length > 0) {
        res.forEach((it) =>
            CpuFreqLimitsStruct.setFreqLimitFrame(
                it,
                5,
                startNS || 0,
                endNS || 0,
    ...
    if (list) {
        for (let i = 0, len = list.length; i < len; i++) {
            ...
            if (
                (it.startNs || 0) + (it.dur || 0) > (startNS || 0) &&
                (it.startNs || 0) < (endNS || 0)
            ) {
                CpuFreqLimitsStruct.setFreqLimitFrame(
                    list[i],
                    5,
                ...
                if (
                    ...
                    (list[i - 1].frame?.width || 0) ==
                        (list[i].frame?.width || 0)
                ) {
                } else {
                    res.push(it);
                }
...
export class CpuFreqLimitsStruct extends BaseStruct {
    ...
    static draw(
        ...
        if (data.frame) {
            ...
            let drawMaxHeight: number = Math.floor(
                ((data.max || 0) * (data.frame.height || 0)) / maxFreq
            );
            let drawMinHeight: number = Math.floor(
                ((data.min || 0) * (data.frame.height || 0)) / maxFreq
            );
            ...
            if (
                ...
                ctx.fillRect(
                    data.frame.x,
                    data.frame.y + data.frame.height - drawMinHeight,
                    width,
                    drawMinHeight
                );
            }
    ...
    static drawArcLine(
        ...
        if (data.frame) {
            ...
            ctx.fillRect(
                data.frame.x,
                data.frame.y + data.frame.height - yStartHeight,
                width,
                drawHeight
            );
            ctx.globalAlpha = this.maxAlpha;
