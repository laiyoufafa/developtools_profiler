...
export class SmapsRender extends Render {
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            ...
            let maxValueName = '';
            if (
                req.params.maxValue != undefined ||
                req.params.maxValueName != undefined
            ) {
...
export class SmapsStruct extends BaseStruct {
    ...
    static draw(
        ...
        if (data.frame) {
            ...
            if (
                data.startNS === SmapsStruct.hoverSmapsStruct?.startNS &&
                isHover
            ) {
                ...
                context2D.lineWidth = 1;
                let drawHeight: number = Math.floor(
                    ((data.value || 0) * (data.frame.height || 0)) / maxValue
                );
                context2D.fillRect(
                    data.frame.x,
                    data.frame.y + data.frame.height - drawHeight + 4,
