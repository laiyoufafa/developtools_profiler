...
export function jank(
    ...
    if (use && res.length > 0) {
        for (let i = 0, len = res.length; i < len; i++) {
            if (
                (res[i].ts || 0) + (res[i].dur || 0) >= startNS &&
                (res[i].ts || 0) <= endNS
            ) {
                JankStruct.setJankFrame(
                    res[i],
                    0,
                    startNS,
                    endNS,
...
export class JankStruct extends BaseStruct {
    ...
    static setJankFrame(
        ...
        if (
            (node.ts || 0) + (node.dur || 0) > startNS &&
            (node.ts || 0) + (node.dur || 0) < endNS
        ) {
            x2 = ns2x(
                (node.ts || 0) + (node.dur || 0),
                startNS,
                endNS,
                totalNS,
                frame
