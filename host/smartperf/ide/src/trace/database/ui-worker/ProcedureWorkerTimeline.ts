...
export function timeline(
    ...
    if (timeRuler == undefined) {
        timeRuler = new TimeRuler(
            canvas,
            ctx,
            new Rect(0, 0, frame.width, 20),
            totalNS
        );
    }
    if (!sportRuler) {
        sportRuler = new SportRuler(
            canvas,
            ctx,
            new Rect(0, 100.5, frame.width, frame.height - 100)
        );
    }
    if (!rangeRuler) {
            ...
            if (y >= 123 && y < 142 && x > 0) {
                let onFlagRange = this.flagList.findIndex((flagObj: Flag) => {
                    let flag_x = Math.round(
                        (this.ruler_w * (flagObj.time - this.range.startNS)) /
...
export class RangeRuler extends Graph {
    ...
    fillX() {
        ...
        this.range.startX =
            (this.range.startNS * (this.frame.width || 0)) / this.range.totalNS;
        this.range.endX =
            (this.range.endNS * (this.frame.width || 0)) / this.range.totalNS;
        ...
        switch (ev.key.toLocaleLowerCase()) {
            ...
            case 'd':
                let animD = () => {
                    ...
                    this.range.startNS +=
                        (this.scale / this.p) * this.currentDuration;
                    this.range.endNS +=
                        (this.scale / this.p) * this.currentDuration;
                    this.fillX();
                    this.draw();
        ...
        switch (ev.key) {
            case 'w':
                let animW = () => {
                    ...
                    this.range.endNS -=
                        ((1 - this.centerXPercentage) * 100 * this.scale) /
                        this.p;
                    this.fillX();
                    this.draw();
                    if (dur < 300) {
