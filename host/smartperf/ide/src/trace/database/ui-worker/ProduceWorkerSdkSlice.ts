...
export class SdkSliceRender extends Render {
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            ...
            SdkSliceStruct.selectSdkSliceStruct =
                req.params.selectSdkSliceStruct;
            for (let re of filter) {
                SdkSliceStruct.draw(req.context, re);
            }
    ...
    sdkSlice(
        ...
        if (use && res.length > 0) {
            for (let index = 0; index < res.length; index++) {
                let item = res[index];
                if (
                    (item.end_ts || 0) > (startNS || 0) &&
                    (item.start_ts || 0) < (endNS || 0)
                ) {
                    SdkSliceStruct.setSdkSliceFrame(
                        res[index],
                        5,
                        startNS || 0,
                        endNS || 0,
...
export class SdkSliceStruct extends BaseStruct {
    ...
    static draw(ctx: CanvasRenderingContext2D, data: SdkSliceStruct) {
        if (data.frame) {
            ...
            if (
                data.start_ts === SdkSliceStruct.hoverSdkSliceStruct?.start_ts
            ) {
                ctx.lineWidth = 1;
                ctx.fillRect(
                    data.frame.x,
