...
export class EnergyStateRender extends Render {
    renderMainThread(
        ...
        if (
            req.maxStateName != 'enable' &&
            req.maxStateName != 'disable' &&
            req.maxStateName != '-1'
        ) {
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            ...
            EnergyStateStruct.selectEnergyStateStruct =
                req.params.selectEnergyStateStruct;
            for (let re of filter) {
    ...
    if (use && res.length > 0) {
        for (let i = 0; i < res.length; i++) {
            ...
            if (
                ...
                EnergyStateStruct.setStateFrame(
                    item,
                    5,
                    startNS || 0,
                    endNS || 0,
    ...
    static draw(
        ...
        if (data.frame) {
            ...
            let drawHeight: number = Math.floor(
                ((data.value || 0) * (data.frame.height || 0)) / maxState
            );
            ...
            if (
                data.startNs ===
                EnergyStateStruct.hoverEnergyStateStruct?.startNs
            ) {
