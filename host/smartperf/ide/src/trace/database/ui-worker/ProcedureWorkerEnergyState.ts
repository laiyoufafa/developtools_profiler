...
export function state(
  ...
  if (use && res.length > 0) {
    for (let i = 0; i < res.length; i++) {
      ...
      if ((item.startNs || 0) + (item.dur || 0) > (startNS || 0) && (item.startNs || 0) < (endNS || 0)) {
        EnergyStateStruct.setStateFrame(item, 5, startNS || 0, endNS || 0, totalNS || 0, frame);
      } else {
        item.frame = null;
      }
    }
  ...
  if (list) {
    for (let index = 0; index < list.length; index++) {
      ...
      if ((item.startNs || 0) + (item.dur || 0) > (startNS || 0) && (item.startNs || 0) < (endNS || 0)) {
        EnergyStateStruct.setStateFrame(list[index], 5, startNS || 0, endNS || 0, totalNS || 0, frame);
        if (
          index > 0 &&
