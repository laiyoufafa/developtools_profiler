...
export class EnergyPowerRender extends Render {
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            req.context.clearRect(
                0,
                0,
                req.canvas.width,
                EnergyPowerStruct.rowHeight
            );
            let arr = filter;
            if (
                arr.length > 0 &&
            ...
            EnergyPowerStruct.selectEnergyPowerStruct =
                req.params.selectEnergyPowerStruct;
...
export function drawLegend(req: any, isDark?: boolean) {
    ...
    for (let index = 0; index < textList.length; index++) {
        ...
        req.context.fillStyle = EnergyPowerStruct.getHistogramColor(
            textList[index]
        );
        req.context.globalAlpha = 1;
        let canvasEndX =
            req.context.canvas.clientWidth - EnergyPowerStruct.OFFSET_WIDTH;
        let textColor = isDark ? '#FFFFFF' : '#333';
        if (index == 0) {
            req!.context.fillRect(
                canvasEndX - EnergyPowerStruct.powerItemNumber * 80,
                12,
                8,
                8
            );
            ...
            req.context.textBaseline = 'middle';
            req.context.fillText(
                textList[index],
                canvasEndX - EnergyPowerStruct.powerItemNumber * 80 + 10,
                18
            );
            EnergyPowerStruct.currentTextWidth =
                canvasEndX -
                EnergyPowerStruct.powerItemNumber * 80 +
                40 +
                text.width;
        } else {
            ...
            req!.context.fillText(
                textList[index],
                EnergyPowerStruct.currentTextWidth + 12,
                18
            );
            EnergyPowerStruct.currentTextWidth =
                EnergyPowerStruct.currentTextWidth + 40 + text.width;
        }
    }
    req.context.fillStyle = '#333';
}
        ...
        for (let index = 0; index < array.length; index++) {
            if (res.length == 0) {
                ...
                if (obj.ts >= leftTime && obj.ts <= rightTime) {
                    ...
                    obj.display =
                        obj.display == 0 ? array[index].display : obj.display;
                    obj.camera =
                        obj.camera == 0 ? array[index].camera : obj.camera;
                    obj.bluetooth =
                        obj.bluetooth == 0
                            ? array[index].bluetooth
                            : obj.bluetooth;
                    obj.flashlight =
                        obj.flashlight == 0
                            ? array[index].flashlight
                            : obj.flashlight;
                    obj.audio = obj.audio ? array[index].audio : obj.audio;
                    obj.wifiscan =
                        obj.wifiscan == 0
                            ? array[index].wifiscan
                            : obj.wifiscan;
                } else {
                    res.push(array[index]);
                }
...
export class EnergyPowerStruct extends BaseStruct {
    ...
    static draw(
        ...
        if (data.frame) {
            ...
            let cpuHeight = this.drawHistogram(
                req,
                data,
                -1,
                data.cpu!,
            ...
            let locationHeight = this.drawHistogram(
                req,
                data,
                cpuHeight,
                data.location!,
            ...
            let gpuHeight = this.drawHistogram(
                req,
                data,
                cpuHeight - locationHeight,
                data.gpu!,
                'GPU',
                row.frame
            );
            ...
            let maxPointY = this.drawPolyline(
                req,
                index,
                data,
                row.frame,
            ...
            if (data.ts === EnergyPowerStruct.hoverEnergyPowerStruct?.ts) {
                ...
                req.context.fillStyle = '#333';
                req!.context.strokeRect(
                    startPointX,
                    maxPointY,
                    frameWidth,
                    req.context.canvas.width - maxPointY
                );
            }
        }
        req!.context.globalAlpha = 1.0;
    ...
    static drawHistogram(
        ...
        let dataHeight: number = Math.floor(
            ((itemValue || 0) * (this.rowHeight - 40)) /
