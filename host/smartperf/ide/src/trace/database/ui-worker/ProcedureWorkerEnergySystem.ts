...
export class EnergySystemRender extends Render {
    renderMainThread(
        ...
        if (EnergySystemStruct.hoverEnergySystemStruct) {
            ...
            EnergySystemStruct.hoverEnergySystemStruct!.power =
                a.power == undefined ? '0' : a.power;
            EnergySystemStruct.hoverEnergySystemStruct!.location =
                a.location == undefined ? '0' : a.location;
        }
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            ...
            EnergySystemStruct.selectEnergySystemStruct =
                req.params.selectEnergySystemStruct;
            for (let re of filter) {
                EnergySystemStruct.draw(req.context, re);
            }
            drawLegend(req);
...
export function drawLegend(req: RequestMessage | any, isDark?: boolean) {
    ...
    for (let index = 0; index < textList.length; index++) {
        ...
        let canvasEndX =
            req.context.canvas.clientWidth - EnergySystemStruct.OFFSET_WIDTH;
        let textColor = isDark ? '#FFFFFF' : '#333';
        if (textList[index] == 'WORKSCHEDULER') {
            req.context.fillRect(
                canvasEndX - EnergySystemStruct.itemNumber * 120,
                12,
                8,
                8
            );
            ...
            req.context.fillText(
                textList[index],
                canvasEndX - EnergySystemStruct.itemNumber * 120 + 10,
                18
            );
            EnergySystemStruct.currentTextWidth =
                canvasEndX -
                EnergySystemStruct.itemNumber * 120 +
                40 +
                text.width;
        } else {
            ...
            req.context.fillText(
                textList[index],
                EnergySystemStruct.currentTextWidth + 12,
                18
            );
            EnergySystemStruct.currentTextWidth =
                EnergySystemStruct.currentTextWidth + 40 + text.width;
        }
    }
    ...
    if (use && res.length > 0) {
        for (let i = 0; i < res.length; i++) {
            ...
            if (
                ...
                EnergySystemStruct.setSystemFrame(
                    item,
                    10,
                    startNS || 0,
                    endNS || 0,
            ...
            if (arr) {
                for (let index = 0; index < arr.length; index++) {
                    ...
                    if (index === arr.length - 1) {
                        ...
                        item.dur =
                            (arr[index + 1].startNs || 0) - (item.startNs || 0);
                    }
                    if (item.count == 0) {
                        item.dur = 0;
                    }
                    if (
                        (item.startNs || 0) + (item.dur || 0) >
                            (startNS || 0) &&
                        (item.startNs || 0) < (endNS || 0)
                    ) {
                        EnergySystemStruct.setSystemFrame(
