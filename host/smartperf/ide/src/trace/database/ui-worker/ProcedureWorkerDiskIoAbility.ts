...
export class DiskIoAbilityRender extends Render {
    ...
    render(req: RequestMessage, list: Array<any>, filter: Array<any>) {
        ...
        if (req.canvas) {
            ...
            DiskAbilityMonitorStruct.selectDiskAbilityStruct =
                req.params.selectDiskAbilityStruct;
            for (let re of filter) {
                DiskAbilityMonitorStruct.draw(
                    req.context,
                    re,
...
export class DiskAbilityMonitorStruct extends BaseStruct {
    ...
    static draw(
        ...
        if (data.frame) {
            ...
            if (
                data.startNS ===
                    DiskAbilityMonitorStruct.hoverDiskAbilityStruct?.startNS &&
                isHover
            ) {
                ...
                context2D.lineWidth = 1;
                let drawHeight: number = Math.floor(
                    ((data.value || 0) * (data.frame.height || 0)) / maxDiskRate
                );
                context2D.fillRect(
                    data.frame.x,
                    data.frame.y + data.frame.height - drawHeight + 4,
