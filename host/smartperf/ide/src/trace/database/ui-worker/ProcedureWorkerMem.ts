        ...
        if (req.canvas) {
            ...
            if (req.isHover) {
                ...
                ProcessMemStruct.hoverProcessMemStruct =
                    req.params.hoverProcessMemStruct;
            }
            for (let re of filter) {
                ProcessMemStruct.draw(req.context, re);
            }
...
function setMemFrame(
    ...
    if ((node.startTime || 0) + (node.duration || 0) >= endNS) {
        ...
        x2 = ns2x(
            (node.startTime || 0) + (node.duration || 0),
            startNS,
            endNS,
            totalNS,
            frame
...
export function mem(
    ...
    if (use && res.length > 0) {
        for (let i = 0, len = res.length; i < len; i++) {
            if (
                (res[i].startTime || 0) + (res[i].duration || 0) > startNS &&
                (res[i].startTime || 0) < endNS
            ) {
    ...
    if (list) {
        for (let i = 0, len = list.length; i < len; i++) {
            let it = list[i];
            if (
                (it.startTime || 0) + (it.duration || 0) > startNS &&
...
export class ProcessMemStruct extends BaseStruct {
    ...
    static draw(ctx: CanvasRenderingContext2D, data: ProcessMemStruct) {
        if (data.frame) {
            ...
            if (
                data.track_id ===
                    ProcessMemStruct.hoverProcessMemStruct?.track_id &&
                data.startTime ===
                    ProcessMemStruct.hoverProcessMemStruct?.startTime
            ) {
                ctx.lineWidth = 1;
                ctx.globalAlpha = 0.6;
