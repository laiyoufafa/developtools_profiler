...
class ProcedurePool {
    static cpuCount = Math.floor(
        (window.navigator.hardwareConcurrency || 4) / 2
    );
    ...
    newThread() {
        let thread: ProcedureThread = new ProcedureThread(
            'trace/database/ui-worker/ProcedureWorker.js',
            { type: 'module' }
        );
    ...
    logicDataThread() {
        let thread: ProcedureThread = new ProcedureThread(
            'trace/database/logic-worker/ProcedureLogicWorker.js',
            { type: 'module' }
        );
        thread.name =
            this.logicDataHandles[this.works.length - this.names.length];
        thread.onmessage = (event: MessageEvent) => {
            thread.busy = false;
