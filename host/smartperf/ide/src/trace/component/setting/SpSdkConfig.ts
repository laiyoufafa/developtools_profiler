...
export class SpSdkConfig extends BaseElement {
    ...
    initElements(): void {
        try {
            let spApplication =
                document.querySelector<HTMLElement>('sp-application');
            let wasmJsonUrl = `https://${window.location.host.split(':')[0]}:${
                window.location.port
            }/application/wasm.json`;
            if (spApplication!.hasAttribute('vs')) {
                wasmJsonUrl = `http://${window.location.host.split(':')[0]}:${
                    window.location.port
                }/wasm.json`;
            }
            fetch(wasmJsonUrl)
        ...
        let switchButton = this.shadowRoot?.querySelector(
            '.config_switch'
        ) as LitSwitch;
        ...
        this.selectConfig = this.shadowRoot?.querySelector<LitAllocationSelect>(
            'lit-allocation-select'
        );
        let inputDiv = this.selectConfig?.shadowRoot?.querySelector(
            '.multipleSelect'
        ) as HTMLDivElement;
        let input =
            this.selectConfig?.shadowRoot?.querySelector<HTMLInputElement>(
                '#singleInput'
            );
        if (input) {
    ...
    initConfig() {
        ...
        for (let key in this.configList.configuration) {
            ...
            switch (this.configList.configuration[key].type) {
                case 'string':
                    if (this.configList.configuration[key].enum) {
                        ...
                        if (this.configList.configuration[key].default) {
                            placeholder =
                                this.configList.configuration[key].default;
                        }
                ...
                case 'number':
                    ...
                    numberInput.oninput = (ev) => {
                        let inputValue = this.checkFloatInput(
                            numberInput.value
                        );
            ...
            if (this.configList.configuration[key].enum) {
                let select = this.shadowRoot!.querySelector<LitSelectV>(
                    `#${key}`
                );
