...
export class SpAllocations extends BaseElement {
    ...
    get pid(): number {
        return Number(this.processId!.value) || -1;
    }

    get unwind(): number {
        return Number(this.unwindEL!.value) || 10;
    }
    ...
    get filter(): number {
        ...
        if (value != "") {
            let unit = this.filterMemoryUnit?.value || "";
            return this.convertToValue(value, unit);
        }
        return 0;
    }
...
<div class="root">
  ...
  <div class="application">
     <span class="inner-font-style">Process Identifier :</span>
     <input id= "pid" class="inputstyle" type="text" placeholder="Enter the pid" oninput="if(this.value > 2147483647) this.value = ''" onkeyup="this.value=this.value.replace(/\\D/g,'')">
  </div>
  <div class="application">
  ...
  <div class="application">
    <span class="inner-font-style">Shared Memory Size :</span> 
    <div>
      <input id = "shareMemory" class="inputstyle" type="text" placeholder="Enter the Shared Memory Size" oninput="if(this.value > 2147483647) this.value = ''" onkeyup="this.value=this.value.replace(/\\D/g,'')" value="4"> 
      <select class="select" id="shareMemoryUnit" >
        <option id= "one_mb" class="select" value="MB">MB</option>
  ...
  <div class="application">
    ...
    <div>
        <input id = "filterSized" class="inputstyle" type="text" placeholder="Enter the Filter Memory Size" oninput="if(this.value > 2147483647) this.value = ''" onkeyup="this.value=this.value.replace(/\\D/g,'')" value="0"> 
        <select class="select" id="filterSizedUnit">
           <option id= "two_mb" class="select" value="MB">MB</option>
           <option id= "two_mb" class="select" value="KB">KB</option>
         </select>
    </div>
  </div>
</div>`;
