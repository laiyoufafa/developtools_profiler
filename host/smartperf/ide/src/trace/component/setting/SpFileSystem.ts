...
export class SpFileSystem extends BaseElement {
    ...
    set startRecord(start: boolean) {
        if (start) {
            ...
            if (
                !this.startFileSystem &&
                !this.startVirtualMemory &&
                !this.startIo
            ) {
    ...
    initElements(): void {
        ...
        this.processInput = this.shadowRoot?.querySelector<LitSelectV>(
            "lit-select-v[title='Process']"
        );
        this.maximum = this.shadowRoot?.querySelector<HTMLInputElement>(
            "input[title='Max Unwind Level']"
        );
                            ...
                            if (conn) {
                                HdcDeviceManager.shellResultAsString(
                                    ...
                                    if (
                                        ...
                                        this.selectProcess!.setAttribute(
                                            'readonly',
                                            'readonly'
                                        );
                                    }
