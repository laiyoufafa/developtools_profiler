...
export class SpRecordPerf extends BaseElement {
            ...
            switch (value.title) {
                ...
                case 'Event List':
                    ...
                    if (selectList.value.length > 0) {
                        perfConfig.eventList = selectList.value.replace(
                            /\s/g,
                            ','
                        );
                    }
    ...
    initElements(): void {
        ...
        this.addOptionButton =
            this.shadowRoot?.querySelector<HTMLButtonElement>('#addOptions');
        ...
        this.processSelect = this.shadowRoot?.querySelector<LitSelectV>(
            "lit-select-v[title='Process']"
        );
        this.processInput =
            this.processSelect?.shadowRoot?.querySelector<HTMLInputElement>(
                'input'
            );
        let querySelector = this.processSelect!.shadowRoot?.querySelector(
            'input'
        ) as HTMLInputElement;
                ...
                if (SpRecordTrace.isVscode) {
                    ...
                    Cmd.execHdcCmd(cmd, (res: string) => {
                        ...
                        if (processData.length > 0 && this.startSamp) {
                            this.processInput!.setAttribute(
                                'readonly',
                                'readonly'
                            );
                        }
                        this.processSelect?.dataSource(
                            processData,
                            'ALL-Process'
                        );
                    });
        ...
        this.cpuSelect = this.shadowRoot?.querySelector<LitSelectV>(
            "lit-select-v[title='CPU']"
        );
        let inputCpu = this.cpuSelect!.shadowRoot?.querySelector(
            'input'
        ) as HTMLInputElement;
        ...
        inputCpu!.addEventListener('mouseup', () => {
            if (SpRecordTrace.serialNumber == '') {
                ...
                if (SpRecordTrace.isVscode) {
                    let cmd = Cmd.formatString(
                        CmdConstant.CMD_GET_CPU_COUNT_DEVICES,
                        [SpRecordTrace.serialNumber]
                    );
                    Cmd.execHdcCmd(cmd, (res: string) => {
                        cpuData = [];
        ...
        this.eventSelect = this.shadowRoot?.querySelector<LitSelectV>(
            "lit-select-v[title='Event List']"
        );
        let inputEvent = this.eventSelect!.shadowRoot?.querySelector(
            'input'
        ) as HTMLInputElement;
        ...
        inputEvent!.addEventListener('click', () => {
            if (SpRecordTrace.serialNumber == '') {
                ...
                if (SpRecordTrace.isVscode) {
                    let cmd = Cmd.formatString(
                        CmdConstant.CMD_GET_HIPERF_EVENTS_DEVICES,
                        [SpRecordTrace.serialNumber]
                    );
                    Cmd.execHdcCmd(cmd, (res: string) => {
                            ...
                            if (conn) {
                                HdcDeviceManager.shellResultAsString(
                                    ...
                                    if (res) {
                                        let eventMap = that.parseEvent(res);
                                        let eventList =
                                            that.getSoftHardWareEvents(
                                                eventMap
                                            );
                                        if (eventList) {
                                            for (let eventListElement of eventList) {
                                                eventData.push(
                                                    eventListElement.trim()
                                                );
                                            }
                                        }
                                        this.eventSelect!.dataSource(
        ...
        this.frequencySetInput =
            this.shadowRoot?.querySelector<HTMLInputElement>(
                "input[title='Frequency']"
            );
        this.offCPUSwitch = this.shadowRoot?.querySelector<LitSwitch>(
            "lit-switch[title='Off CPU']"
        );
        this.callSelect = this.shadowRoot?.querySelector<LitSelect>(
            "lit-select[title='Call Stack']"
        );
    ...
    parseEvent(eventListResult: string): Map<string, Array<string>> {
        let eventMap: Map<string, Array<string>> = new Map<
            string,
            Array<string>
        >();
        let events: Array<string> = [];
        let type: string = '';
        let lineValues: string[] = eventListResult
            .replace(/\r\n/g, '\r')
            .replace(/\n/g, '\r')
            .split(/\r/);
        for (let line of lineValues) {
            if (line.startsWith('Supported')) {
                ...
                type = line
                    .substring(
                        line.indexOf(startSign) + startSign.length,
                        line.lastIndexOf(':')
                    )
                    .trim();
                events = new Array();
