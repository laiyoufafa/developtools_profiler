...
export class SpRecyclerSystemTrace extends BaseElement {
    ...
    initElements(): void {
        this.rowsEL =
            this.shadowRoot?.querySelector<TraceRowRecyclerView>('.rows');
        ...
        document?.addEventListener('flag-change', (event: any) => {
            ...
            this.timerShaftEL?.modifyFlagList(
                event.detail.type,
                event.detail.flagObj
            );
            if (flag.hidden) {
        ...
        new ResizeObserver((entries) => {
            let width =
                ...
                1 -
                SpRecyclerSystemTrace.scrollViewWidth;
            requestAnimationFrame(() => {
                this.timerShaftEL?.updateWidth(width);
                this.shadowRoot!.querySelectorAll<TraceRow<any>>(
                    'trace-row'
                ).forEach((it) => it.updateWidth(width));
            });
        }).observe(this);
        // @ts-ignore
        new ResizeObserver((entries) => {
            let width =
                this.clientWidth - 1 - SpRecyclerSystemTrace.scrollViewWidth;
            requestAnimationFrame(() => {
                this.timerShaftEL?.updateWidth(width);
                this.shadowRoot!.querySelectorAll<TraceRow<any>>(
                    'trace-row'
    ...
    getVisibleRows(): Array<TraceRow<any>> {
        return [
            ...this.rowsEL!.shadowRoot!.querySelectorAll<TraceRow<any>>(
                'trace-row'
            ),
        ];
    }
    ...
    documentOnMouseMove = (ev: MouseEvent) => {
        ...
        rows.forEach((tr) => {
            ...
            let y =
                ev.offsetY -
                (tr.canvasContainer?.offsetTop || 0) +
                (this.rowsEL?.scrollTop || 0);
            //判断鼠标是否在当前 trace-row
            if (
                x > tr.frame.x &&
                x < tr.frame.x + tr.frame.width &&
                y > tr.frame.y &&
                y < tr.frame.y + tr.frame.height
            ) {
                this.hoverStructNull();
                if (tr.rowType === TraceRow.ROW_TYPE_CPU) {
                    ...
                    if (CpuStruct.hoverCpuStruct) {
                        tr.tip = `<span>P：${
                            CpuStruct.hoverCpuStruct.processName || 'Process'
                        } [${
                            CpuStruct.hoverCpuStruct.processId
                        }]</span><span>T：${CpuStruct.hoverCpuStruct.name} [${
                            CpuStruct.hoverCpuStruct.tid
                        }]</span>`;
                    }
                    ...
                    if (CpuFreqStruct.hoverCpuFreqStruct) {
                        tr.tip = `<span>${ColorUtils.formatNumberComma(
                            CpuFreqStruct.hoverCpuFreqStruct.value!
                        )} kHz</span>`;
                    }
                    tr.setTipLeft(x, CpuFreqStruct.hoverCpuFreqStruct);
                } else if (tr.rowType === TraceRow.ROW_TYPE_THREAD) {
                    ThreadStruct.hoverThreadStruct = tr.onMouseHover(
                        x,
                        y,
                        false
                    );
                    ...
                    if (HeapStruct.hoverHeapStruct) {
                        tr.tip = `<span>${ColorUtils.formatNumberComma(
                            HeapStruct.hoverHeapStruct.heapsize!
                        )} byte</span>`;
                    }
    ...
    documentOnClick = (ev: MouseEvent) => {
        ...
        this.rowsEL
            ...
            .forEach((it) => (it.rangeSelect = undefined));
    ...
    connectedCallback() {
         ...
         * 监听时间轴区间变化
         */
        this.timerShaftEL?.addEventListener(
            'range-change',
    ...
    loadDatabaseUrl(url: string, complete?: Function) {
        this.init({ url: url }).then(() => {
            ...
            this.rowsEL?.querySelectorAll('trace-row').forEach((it: any) => {
                ...
                if (
                    top + it.clientHeight > scrollTop &&
                    top + it.clientHeight <
                        scrollTop + scrollHeight + it.clientHeight
                ) {
    ...
    initCpu = async () => {
        ...
        if (array && array.length > 0 && array[0]) {
            ...
            for (let i1 = 0; i1 < CpuStruct.cpuCount; i1++) {
                ...
                traceRow.frame = new Rect(
                    0,
                    0,
                    this.rowsEL.clientWidth - 248,
                    traceRow.rowHeight
                );
                traceRow.name = `Cpu ${cpuId}`;
                traceRow.supplier = () =>
                    queryCpuData(cpuId, 0, this.timerShaftEL?.totalNS || 0);
    ...
    initCpuFreq = async () => {
        ...
        let math = () => {
            ...
            if (CpuFreqStruct.maxFreq > 0) {
                let log10: number = Math.ceil(
                    Math.log10(CpuFreqStruct.maxFreq)
                );
                let pow10: number = Math.pow(10, log10);
                let afterCeil: number =
                    Math.ceil(CpuFreqStruct.maxFreq / (pow10 / 4)) *
                    (pow10 / 4);
                CpuFreqStruct.maxFreq = afterCeil;
                let unitIndex: number = Math.floor(log10 / 3);
                sb = `${afterCeil / Math.pow(10, unitIndex * 3)}${
                    units[unitIndex + 1]
                }hz`;
            }
                ...
                if (fpsRow.dataList) {
                    for (let i = 0; i < fpsRow.dataList.length; i++) {
                        ...
                        if (i === fpsRow.dataList.length - 1) {
                            ...
                            it.dur =
                                (fpsRow.dataList[i + 1].startNS || 0) -
                                (it.startNS || 0);
                        }
                        if (
                            (it.startNS || 0) + (it.dur || 0) >
                            ...
                            if (
                                i > 0 &&
                                (fpsRow.dataList[i - 1].frame?.x || 0) ==
                                    (fpsRow.dataList[i].frame?.x || 0) &&
                                (fpsRow.dataList[i - 1].frame?.width || 0) ==
                                    (fpsRow.dataList[i].frame?.width || 0)
                            ) {
                            } else {
    ...
    initProcess = async () => {
        ...
        for (let i = 0; i < processList.length; i++) {
            ...
            processRow.frame = new Rect(
                0,
                0,
                this.rowsEL.clientWidth - 248,
                processRow.rowHeight
            );
