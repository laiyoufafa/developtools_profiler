...
export class SpQuerySQL extends BaseElement {
    ...
    initElements(): void {
        ...
        this.selector = this.shadowRoot?.querySelector(
            '.sql-select'
        ) as HTMLTextAreaElement;
        this.queryTableEl = new LitTable();
        this.querySize = this.shadowRoot?.querySelector(
            '.query_size'
        ) as HTMLElement;
        ...
        let htmlDivElement = this.queryTableEl.shadowRoot?.querySelector(
            '.table'
        ) as HTMLDivElement;
        ...
        let copyButtonEl = this.shadowRoot?.querySelector(
            '#copy-button'
        ...
        let closeButtonEl = this.shadowRoot?.querySelector(
            '#close-button'
        ) as HTMLButtonElement;
    ...
    freshTableHeadResizeStyle() {
        let th =
            this.queryTableEl!.shadowRoot?.querySelector<HTMLDivElement>('.th');
        if (th) {
            let td = th.querySelectorAll<HTMLDivElement>('.td');
            let firstChild =
                this.queryTableEl!.shadowRoot?.querySelector<HTMLDivElement>(
                    '.body'
                )!.firstElementChild;
            if (firstChild) {
                let bodyList =
                    firstChild.querySelectorAll<HTMLDivElement>('.td');
        ...
        if (event.ctrlKey && event.keyCode == 13) {
            ...
            this.getInputSqlResult(this.selector!.value).then((resultList) => {
                ...
                setTimeout(() => {
                    ...
                    info(
                        'metric query Sql result Data size is: ',
                        this.statDataArray!.length
                    );
                    this.initData();
    ...
    async initMetricData(): Promise<any> {
        ...
        if (this.queryText == '' || this.queryText == null) {
            ...
            if (this.querySize) {
                this.querySize!.textContent =
                    'Query result - ' + statList.length + ' counts';
            }
    ...
    checkSafetySelectSql(): boolean {
        ...
        if (split) {
            this.querySqlErrorText =
                'Error: Incomplete query statement:  ' + this.selector!.value;
            this.isSupportSql = false;
        ...
        if (this.isSupportSql) {
            let sqlField =
                this.keyList?.length == 0
                    ? '*'
                    : this.keyList?.toLocaleString();
            this.querySize!.textContent =
                'Query result - ' + this.statDataArray.length + ' counts';
        } else {
            this.querySize!.textContent = this.querySqlErrorText;
        }

        let queryHeadStyle: HTMLDivElement | undefined | null =
            this.queryTableEl?.shadowRoot?.querySelector(
                'div.th'
            ) as HTMLDivElement;
        if (queryHeadStyle && queryHeadStyle.hasChildNodes()) {
            for (
                let index = 0;
                index < queryHeadStyle.children.length;
                index++
            ) {
                // @ts-ignore
    ...
    attributeChangedCallback(name: string, oldValue: string, newValue: string) {
        ...
        if (queryDataSty && queryDataSty.hasChildNodes()) {
            for (let index = 0; index < queryDataSty.children.length; index++) {
                // @ts-ignore
                queryDataSty.children[index].style.backgroundColor =
                    'var(--dark-background5,#F6F6F6)';
            }
        }
    }
