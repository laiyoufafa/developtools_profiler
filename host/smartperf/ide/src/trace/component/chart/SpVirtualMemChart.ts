...
import {
    VirtualMemoryRender,
    VirtualMemoryStruct,
} from '../../database/ui-worker/ProcedureWorkerVirtualMemory.js';
...
export class SpVirtualMemChart {
    ...
    async init() {
        ...
        array.forEach((it, idx) =>
            this.initVirtualMemoryRow(folder, it.id, it.name, idx)
        );
    }

    initVirtualMemoryRow(
        folder: TraceRow<any>,
        id: number,
        name: string,
        idx: number
        ...
        row.onThreadHandler = (useCache) => {
            ...
            (
                renders['virtual-memory-cell'] as VirtualMemoryRender
            ).renderMainThread(
                {
                    context: context,
                    useCache: useCache,
