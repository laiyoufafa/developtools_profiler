...
export class SpCpuChart {
    ...
    async init() {
        ...
        if (array && array.length > 0 && array[0]) {
            ...
            for (let i1 = 0; i1 < CpuStruct.cpuCount; i1++) {
                ...
                traceRow.favoriteChangeHandler =
                        ...
                        res.forEach((it, i, arr) => {
                            if (i !== arr.length - 1) {
                                if (
                                    ...
                                    it.dur =
                                        arr[i + 1]!.startTime! - it.startTime!;
                                    it.nofinish = true;
                                }
                            } else {
                                if (it.dur == -1) {
                                    it.dur =
                                        TraceRow.range!.endNS - it.startTime!;
                                    it.nofinish = true;
                                }
                            }
                        });
                ...
                traceRow.onThreadHandler = (
                    useCache: boolean,
                    buf: ArrayBuffer | undefined | null
                ) => {
    ...
    initProcessThreadStateData = async (progress: Function) => {
        ...
        info(
            'The time to load the first ProcessThreadState data is: ',
            durTime
        );
    };
