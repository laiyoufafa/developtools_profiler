...
import {
    HiperfCpuRender,
    HiPerfCpuStruct,
} from '../../database/ui-worker/ProcedureWorkerHiPerfCPU.js';
import {
    HiperfThreadRender,
    HiPerfThreadStruct,
} from '../../database/ui-worker/ProcedureWorkerHiPerfThread.js';
import {
    HiperfProcessRender,
    HiPerfProcessStruct,
} from '../../database/ui-worker/ProcedureWorkerHiPerfProcess.js';
import { info } from '../../../log/Log.js';
import {
    HiperfEventRender,
    HiPerfEventStruct,
} from '../../database/ui-worker/ProcedureWorkerHiPerfEvent.js';
...
import {
    HiperfReportRender,
    HiPerfReportStruct,
} from '../../database/ui-worker/ProcedureWorkerHiPerfReport.js';
...
export class SpHiPerf {
    ...
    async init() {
        ...
        this.maxCpuId =
            this.cpuData.length > 0 ? this.cpuData[0].cpu_id : -Infinity;
    ...
    getStringResult(s: string = '') {
        ...
        SpHiPerf.stringResult = {
            ...
            existF: sF !== -1,
            fValue: Number(
                (1000 / (sF !== -1 ? parseInt(list[sF + 1]) : 1000)).toFixed(1)
            ),
        };
    }
    ...
    async initCpuMerge() {
        ...
        row.focusHandler = () =>
            this.hoverTip(row, HiPerfCpuStruct.hoverStruct);
        row.onThreadHandler = (useCache) => {
    ...
    async initReport() {
        this.eventTypeList.forEach((it, index) => {
            ...
            fold.onThreadHandler = (useCache) => {
                let context = fold.collect
                    ? this.trace.canvasFavoritePanelCtx!
                    : this.trace.canvasPanelCtx!;
                fold.canvasSave(context);
                (
                    renders['HiPerf-Report-Fold'] as HiperfReportRender
                ).renderMainThread(
                    {
                        context: context,
                        useCache: useCache,
            ...
            for (let i = 0; i <= this.maxCpuId; i++) {
                ...
                row.focusHandler = () =>
                    this.hoverTip(row, HiPerfEventStruct.hoverStruct);
                row.onThreadHandler = (useCache) => {
                    let context = fold.collect
                        ? this.trace.canvasFavoritePanelCtx!
                        : this.trace.canvasPanelCtx!;
                    fold.canvasSave(context);
                    (
                        renders['HiPerf-Report-Event'] as HiperfEventRender
                    ).renderMainThread(
                        {
                            context: context,
                            useCache: useCache,
    ...
    async initProcess() {
        Reflect.ownKeys(this.group).forEach((key, index) => {
            ...
            row.focusHandler = () =>
                this.hoverTip(row, HiPerfProcessStruct.hoverStruct);
            row.onThreadHandler = (useCache) => {
                ...
                if (row.expansion) {
                    ...
                    (
                        renders['HiPerf-Process'] as HiperfProcessRender
                    ).renderMainThread(
                        {
                            context: context,
                            useCache: useCache,
            ...
            array.forEach((thObj, thIdx) => {
                ...
                thread.focusHandler = () =>
                    this.hoverTip(thread, HiPerfThreadStruct.hoverStruct);
                thread.onThreadHandler = (useCache) => {
                    let context = thread.collect
                        ? this.trace.canvasFavoritePanelCtx!
                        : this.trace.canvasPanelCtx!;
                    thread.canvasSave(context);
                    (
                        renders['HiPerf-Thread'] as HiperfThreadRender
                    ).renderMainThread(
                        {
                            context: context,
                            useCache: useCache,
    ...
    hoverTip(
        ...
        if (struct) {
            ...
            if (num > 0) {
                ...
                let perfCall = perfDataQuery.callChainMap.get(
                    struct.callchain_id || 0
                );
                tip = `<span>${perfCall ? perfCall.name : ''} (${
                    perfCall ? perfCall.depth : '0'
                } other frames)</span>`;
            }
        }
