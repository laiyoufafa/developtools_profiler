...
import {
    queryClockData, queryClockFrequency, queryClockState,
    queryScreenState
    ...
    async initData(folder:TraceRow<any>){
        ...
        for (let i = 0; i < clockList.length; i++) {
            ...
            traceRow.focusHandler = (ev)=>{
                this.trace?.displayTip(traceRow, ClockStruct.hoverClockStruct, `<span>${ColorUtils.formatNumberComma(ClockStruct.hoverClockStruct?.value!)}</span>`);
            };
                ...
                (renders["clock"] as ClockRender).renderMainThread(
                    {
                        ...
                        maxValue: maxValue,
                        index:i,
                        maxName:(isState||isScreenState)?maxValue.toString():Utils.getFrequencyWithUnit(maxValue/1000).maxFreqName
                    },
                    traceRow
                );
