...
import {
    ClockRender,
    ClockStruct,
} from '../../database/ui-worker/ProcedureWorkerClock.js';
...
export class SpClockChart {
    ...
    async initData(folder: TraceRow<any>) {
        ...
        ClockStruct.maxValue = clockList
            .map((item) => item.num)
            ...
            traceRow.focusHandler = (ev) => {
                this.trace?.displayTip(
                    ...
                    `<span>${ColorUtils.formatNumberComma(
                        ClockStruct.hoverClockStruct?.value!
                    )}</span>`
                );
            };
                ...
                (renders['clock'] as ClockRender).renderMainThread(
                    {
                        ...
                        maxName:
                            isState || isScreenState
                                ? maxValue.toString()
                                : Utils.getFrequencyWithUnit(maxValue / 1000)
                                      .maxFreqName,
                    },
                    traceRow
                );
