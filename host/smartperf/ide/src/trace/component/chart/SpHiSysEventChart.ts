...
import {
    EnergyAnomalyRender,
    EnergyAnomalyStruct,
} from '../../database/ui-worker/ProcedureWorkerEnergyAnomaly.js';
import {
    EnergySystemStruct,
    EnergySystemRender,
} from '../../database/ui-worker/ProcedureWorkerEnergySystem.js';
import {
    EnergyPowerStruct,
    EnergyPowerRender,
} from '../../database/ui-worker/ProcedureWorkerEnergyPower.js';
import {
    EnergyStateStruct,
    EnergyStateRender,
} from '../../database/ui-worker/ProcedureWorkerEnergyState.js';
...
export class SpHiSysEventChart {
    ...
    private initEnergyRow = async () => {
        ...
        if (configAppName.length > 0 && appNameFromTable.length > 0) {
            ...
            if (name != null) {
                let filterList = appNameFromTable.filter(
                    (appNameFromTableElement) => {
                        return (
                            appNameFromTableElement.string_value?.trim() == name
                        );
                    }
                );
        ...
        let appNameList =
            this.energyTraceRow?.shadowRoot!.querySelector<LitPopover>(
                '#appNameList'
            );
        let addFlag = false;
        appNameList?.addEventListener('click', () => {
            ...
            if (!addFlag) {
                for (let index = 0; index < appNameFromTable.length; index++) {
                    ...
                    itemDiv!.append(div);
                    let appList =
                        this.energyTraceRow?.shadowRoot!.querySelectorAll<HTMLInputElement>(
        ...
        this.energyTraceRow.favoriteChangeHandler =
            this.trace.favoriteChangeHandler;
        this.energyTraceRow.selectChangeHandler =
            this.trace.selectChangeHandler;
        this.energyTraceRow.supplier = () =>
        ...
        this.energyTraceRow.onThreadHandler = (useCache) => {
            ...
            if (this.energyTraceRow!.expansion) {
                this.trace.canvasPanelCtx?.clearRect(
                    0,
                    0,
                    this.energyTraceRow!.frame.width,
                    this.energyTraceRow!.frame.height
                );
            } else {
    ...
    private initAnomaly = async () => {
        ...
        anomalyTraceRow.focusHandler = () => {
            this.trace?.displayTip(
                ...
                `<span>AnomalyName:${
                    EnergyAnomalyStruct.hoverEnergyAnomalyStruct?.eventName ||
                    ''
                }</span>`
            );
        };
        anomalyTraceRow.onThreadHandler = (useCache) => {
            let context = anomalyTraceRow.collect
    ...
    private initSystem = async () => {
        ...
        let element = systemTraceRow.shadowRoot?.querySelector(
            '.root'
        ) as HTMLDivElement;
        ...
        systemTraceRow.onThreadHandler = (useCache) => {
            let context = systemTraceRow.collect
                ? this.trace.canvasFavoritePanelCtx!
                : this.trace.canvasPanelCtx!;
    ...
    private initPower = async () => {
        ...
        let element = powerTraceRow.shadowRoot?.querySelector(
            '.root'
        ) as HTMLDivElement;
        ...
        powerTraceRow.onThreadHandler = (useCache) => {
            let context = powerTraceRow.collect
                ? this.trace.canvasFavoritePanelCtx!
                : this.trace.canvasPanelCtx!;
    ...
    private initState = async () => {
        ...
        for (let index = 0; index < stateList.length; index++) {
            ...
            if (
                ...
                maxStateData[0].type.toLocaleLowerCase() !=
                    'bluetooth_br_switch_state'
            ) {
            ...
            stateTraceRow.favoriteChangeHandler =
                this.trace.favoriteChangeHandler;
            ...
            stateTraceRow.supplier = () =>
                Promise.all([
                    ...
                    let stateInitValue =
                        initValueList[index] == 'nocolumn' ? [] : result[0];
                    return stateInitValue.concat(result[1]);
                });
            stateTraceRow.focusHandler = () => {
                let tip = '';
                if (
                    EnergyStateStruct.hoverEnergyStateStruct
                        ?.type!.toLocaleLowerCase()
                        .includes('state')
                ) {
                    tip = `<span>Switch Status: ${
                        EnergyStateStruct.hoverEnergyStateStruct?.value == 1
                            ? 'disable'
                            : 'enable'
                    }</span>`;
                    if (
                        EnergyStateStruct.hoverEnergyStateStruct?.type!.toLocaleLowerCase() ==
                        'bluetooth_br_switch_state'
                    ) {
                    ...
                    tip = `<span>value: ${
                        EnergyStateStruct.hoverEnergyStateStruct?.value || 0
                    }</span>`;
                }
                this.trace?.displayTip(
                    stateTraceRow,
            ...
            stateTraceRow.onThreadHandler = (useCache) => {
                let context = stateTraceRow.collect
                    ? this.trace.canvasFavoritePanelCtx!
                    : this.trace.canvasPanelCtx!;
