...
export class TraceSheet extends BaseElement {
    ...
    displayTab<T>(...names: string[]): T {
        ...
        let litTabpane = this.shadowRoot?.querySelector<LitTabpane>(
            `#tabs lit-tabpane[id='${names[0]}']`
        );
        this.shadowRoot
            ?.querySelector<LitTabs>('#tabs')
            ?.activePane(litTabpane!.key);
    ...
    initElements(): void {
        ...
        this.litTabs!.addEventListener('close-handler', () => {
            Reflect.ownKeys(tabConfig)
                ...
                .forEach((id) => {
                    ...
                    let pane = this.shadowRoot!.querySelector<LitTabpane>(
                        `#${id as string}`
                    );
                    if (element.require) {
            ...
            this.litTabs?.activeByKey(
                `${this.getPaneByID(this.currentPaneID).key}`
            );
        });
        this.getComponentByID<any>('box-spt')!.addEventListener(
            'row-click',
            this.rowClickHandler.bind(this)
        );
        this.getComponentByID<any>('box-pts')!.addEventListener(
            'row-click',
            this.rowClickHandler.bind(this)
        );
        this.getComponentByID<any>('box-cs')!.addEventListener(
            'row-click',
            this.rowClickHandler.bind(this)
        );
        this.getComponentByID<any>('box-ts')!.addEventListener(
            'row-click',
            this.rowClickHandler.bind(this)
        );
        this.getComponentByID<any>('box-native-statstics')!.addEventListener(
            'row-click',
            (e: any) => {
        ...
        this.getComponentByID<any>(
            'box-virtual-memory-statistics'
        )!.addEventListener('row-click', (e: any) => {
        ...
        this.getComponentByID<any>('box-io-tier-statistics')!.addEventListener(
            'row-click',
            (e: any) => {
                ...
                this.litTabs?.activeByKey(pane.key);
                if (e.detail.path) {
                    (pane.children.item(0) as any)!.fromStastics(
                        this.selection
                    );
                }
            }
    ...
    connectedCallback() {
        this.nav = this.shadowRoot
            ?.querySelector('#tabs')
            ?.shadowRoot?.querySelector('.tab-nav-container');
        ...
        let timerShaft: HTMLDivElement | undefined | null =
            this.parentElement?.querySelector('.timer-shaft');
        let spacer: HTMLDivElement | undefined | null =
            this.parentElement?.querySelector('.spacer');
        let rowsPaneEL: HTMLDivElement | undefined | null =
            this.parentElement?.querySelector('.rows-pane');
        ...
        tabsPackUp!.onclick = () => {
            ...
            if (tabsPackUp!.name == 'down') {
                tabs!.style.height = navRoot!.offsetHeight + 'px';
                litTabpane!.forEach(
                    (node: HTMLDivElement) => (node!.style.height = '0px')
                );
                tabsPackUp!.name = 'up';
    ...
    displayThreadData = (
        ...
        this.displayTab<TabPaneCurrentSelection>(
            'current-selection'
        ).setThreadData(data, scrollCallback, scrollWakeUp);
    ...
    displayFuncData = (data: FuncStruct, scrollCallback: Function) =>
        this.displayTab<TabPaneCurrentSelection>(
            'current-selection'
        ).setFunctionData(data, scrollCallback);
    displayCpuData = (
        data: CpuStruct,
        ...
        this.displayTab<TabPaneCurrentSelection>(
            'current-selection'
        ).setCpuData(data, callback, scrollCallback);
    displayJankData = (
        data: JankStruct,
        ...
        this.displayTab<TabPaneCurrentSelection>(
            'current-selection'
        ).setJankData(data, callback, scrollCallback);
    displayFlagData = (flagObj: Flag) =>
        this.displayTab<TabPaneFlag>('box-flag').setFlagObj(flagObj);
    displayFreqData = () =>
        (this.displayTab<TabPaneCurrentSelection>('box-freq').data =
            CpuFreqStruct.selectCpuFreqStruct);
    displayCpuStateData = () =>
        (this.displayTab<TabPaneCurrentSelection>('cpu-state-click').data =
            CpuStateStruct.selectStateStruct);
    displayFreqLimitData = () =>
        (this.displayTab<TabPaneCurrentSelection>('box-freq-limit').data =
    ...
    rangeSelect(selection: SelectionParam): boolean {
            ...
            .forEach((id) => {
                ...
                if (element.require) {
                    if (element.require(selection)) {
                        ...
                        this.shadowRoot!.querySelector<LitTabpane>(
                            `#${id as string}`
                        )!.hidden = true;
                    }
                } else {
        ...
        let firstPane = this.shadowRoot!.querySelector<LitTabpane>(
            `lit-tabpane[hidden='false']`
        );
        if (firstPane) {
