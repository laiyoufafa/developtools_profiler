...
export class Utils {
    ...
    constructor() {
        ...
        Utils.statusMap.set(
            'DK-NIO',
            'Uninterruptible Sleep(non-IO) + Wake Kill'
        );
    ...
    public static getEndState(state: string): string {
        if (Utils.getInstance().getStatusMap().has(state)) {
            return (
                Utils.getInstance().getStatusMap().get(state) || 'Unknown State'
            );
        } else {
            if ('' == state || state == null) {
                return '';
    ...
    public static getTimeStampHMS(ns: number): string {
        ...
        if (currentNs >= hour1) {
            res +=
                this.getCompletionTime(Math.floor(currentNs / hour1), 2) + ':';
            currentNs = currentNs - Math.floor(currentNs / hour1) * hour1;
        }
        if (currentNs >= minute1) {
            res +=
                this.getCompletionTime(Math.floor(currentNs / minute1), 2) +
                ':';
            currentNs = currentNs - Math.floor(ns / minute1) * minute1;
        }
        if (currentNs >= second1) {
            res +=
                this.getCompletionTime(Math.floor(currentNs / second1), 2) +
                ':';
            currentNs = currentNs - Math.floor(currentNs / second1) * second1;
        } else {
            res += '00:';
        ...
        if (currentNs >= millisecond1) {
            res +=
                this.getCompletionTime(
                    Math.floor(currentNs / millisecond1),
                    3
                ) + '.';
            currentNs =
                currentNs - Math.floor(currentNs / millisecond1) * millisecond1;
        ...
        if (currentNs >= microsecond1) {
            res +=
                this.getCompletionTime(
                    Math.floor(currentNs / microsecond1),
                    3
                ) + '.';
            currentNs =
                currentNs - Math.floor(currentNs / microsecond1) * microsecond1;
