...
export class RangeSelect {
    ...
    public selectHandler:
        | ((ds: Array<TraceRow<any>>, refreshCheckBox: boolean) => void)
        | undefined;
    ...
    constructor(trace: SpSystemTrace | null | undefined) {
        ...
        this.timerShaftDragEL = this.timerShaftEL?.shadowRoot?.querySelector(
    ...
    mouseDown(ev: MouseEvent) {
        ...
        if (this.isInRowsEl(ev)) {
            ...
            if (this.isInSpacerEL(ev)) {
                ...
                this.startY2 =
                    ev.pageY -
                    this.spacerEL.getBoundingClientRect().top -
                    this.rowsPaneEL!.scrollTop;
            } else {
                this.startY =
                    ev.pageY -
                    this.rowsEL!.getBoundingClientRect().top +
                    this.spacerEL.getBoundingClientRect().height;
                this.startY2 =
                    ev.pageY -
                    this.spacerEL!.getBoundingClientRect().top -
                    this.rowsPaneEL!.scrollTop;
    ...
    isTouchMark(ev: MouseEvent): boolean {
        ...
        return (
            notTimeHeight &&
            (this.rangeTraceRow?.isNotEmpty() ?? false) &&
            !this.isMouseDown
        );
    }
    ...
    mouseOut(ev: MouseEvent) {
        if (this.drag) {
            this.endX =
                this.rowsEL!.getBoundingClientRect().right -
                this.rowsEL!.getBoundingClientRect().left -
                248;
            if (this.isInSpacerEL(ev)) {
                this.endY = 0;
                this.endY2 =
    ...
    mouseMove(rows: Array<TraceRow<any>>, ev: MouseEvent) {
        if (this.isTouchMark(ev) && TraceRow.rangeSelectObject) {
            ...
            let mouseX =
                ev.pageX - this.rowsPaneEL!.getBoundingClientRect().left - 248;
        ...
        if (this.isHover && this.isMouseDown) {
            ...
            this.rangeTraceRow = rows.filter((it) => {
                if (it.rangeSelect) {
                    if (!rangeSelect) {
                        ...
                        let markA =
                            this.movingMark == 'markA'
                                ? mouseX
                                : this.mark.startMark;
                        let markB =
                            this.movingMark == 'markB'
                                ? mouseX
                                : this.mark.endMark;
                        ...
                        rangeSelect.startNS = Math.floor(
                            ((TraceRow.range!.endNS - TraceRow.range!.startNS) *
                                startX) /
                                it.frame.width +
                                TraceRow.range!.startNS!
                        );
                        rangeSelect.endNS = Math.floor(
                            ((TraceRow.range!.endNS - TraceRow.range!.startNS) *
                                endX) /
                                it.frame.width +
                                TraceRow.range!.startNS!
            ...
            this.timerShaftEL!.sportRuler!.isRangeSelect =
                (this.rangeTraceRow?.length || 0) > 0;
        ...
        this.rangeTraceRow = rows.filter((it) => {
            ...
            if (it.collect) {
                ...
                rt = new Rect(
                    xMin,
                    Math.min(this.startY2, this.endY2),
                    xMax - xMin,
                    Math.abs(this.startY2 - this.endY2)
                );
            } else {
                bound = it.getBoundingClientRect();
                ...
                itRect = Rect.getIntersect(
                    ...
                    new Rect(
                        rowsRect.x,
                        rowsRect.y + spacerRect.height,
                        rowsRect.width,
                        rowsRect.height - spacerRect.height
                    )
                );
