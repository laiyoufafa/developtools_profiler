...
export class TabPaneSystemDetails extends BaseElement {
    ...
    initElements(): void {
        this.boxDetails =
            this.shadowRoot?.querySelector<HTMLDivElement>('.box-details');
        this.tbl = this.shadowRoot?.querySelector<LitTable>('#tb-system-data');
        this.detailsTbl = this.shadowRoot?.querySelector<LitTable>(
            '#tb-system-details-data'
        );
    ...
    convertData(data: SystemDetailsEnergy) {
        ...
        this.detailsTbl!.shadowRoot?.querySelectorAll<HTMLDivElement>(
            '.td'
        ).forEach((td) => {
    ...
    queryDataByDB(val: SelectionParam | any) {
        Promise.all([
            ...
            let systemWorkData = this.getSystemWorkData(
                result[0],
                val.leftNs,
                val.rightNs
            );
            ...
            let systemLocationData = this.getSystemLocationData(
                result[2],
                val.leftNs
            );
            ...
            this.boxDetails!.style.width = '100%';
            this.tbl?.shadowRoot
                ?.querySelectorAll<HTMLDivElement>('.td')
                .forEach((td) => {
                    td.style.fontSize = '14px';
                    if (
                        td.getAttribute('title') === 'Event Name' ||
                        td.getAttribute('title') === 'Time'
                    ) {
        ...
        lifeCycleData.forEach((life: any) => {
            if (life.endData.ts >= leftNs) {
                ...
                midData.forEach((rang: any, index: number) => {
                    if (
                        rang.eventName.indexOf('WORK_STOP') > -1 &&
                        rang.ts >= leftNs
                    ) {
                        resultData.push(life.startData);
                        if (
                            index - 1 >= 0 &&
                            midData[index - 1].eventName.indexOf('WORK_START') >
                                -1
                        ) {
    ...
    private getConvertData(data: Array<any>) {
        ...
        data.forEach((item: any) => {
            if (it[item.ts + item.eventName] == undefined) {
                ...
                it[item.ts + item.eventName][item.appKey.toLocaleLowerCase()] =
                    item.appValue;
            } else {
                it[item.ts + item.eventName][item.appKey.toLocaleLowerCase()] =
                    item.appValue;
