...
export class TabPaneIrqCounter extends BaseElement {
    ...
    set data(val: SelectionParam | any) {
        ...
        for (let key of collect.keys()) {
            ...
            sd.avgDuration = Utils.getProbablyTime(
                sd.wallDuration / parseInt(sd.count)
            );
            dataSource.push(sd);
        }
    ...
    createSelectCounterData(name: string, list: Array<any>): SelectionData {
        ...
        if (list.length > 0) {
            ...
            selectData.wallDurationFormat = Utils.getProbablyTime(
                selectData.wallDuration
            );
        }
        return selectData;
    }
        ...
        if (type == 0) {
            ...
            arr.sort((a, b): number => {
                if (key == 'wallDurationFormat') {
                    ...
                    if (type == 1) {
                        return (
                            a.wallDuration / parseInt(a.count) -
                            b.wallDuration / parseInt(b.count)
                        );
                    } else {
                        return (
                            b.wallDuration / parseInt(b.count) -
