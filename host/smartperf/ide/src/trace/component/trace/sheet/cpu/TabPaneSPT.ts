...
import {
    SPT,
    StateProcessThread,
} from '../../../../bean/StateProcessThread.js';
...
export class TabPaneSPT extends BaseElement {
    ...
    getDataBySPT(leftNs: number, rightNs: number, source: Array<SPT>) {
        let statesMap: Map<string, StateProcessThread> = new Map<
            string,
            StateProcessThread
        >();
        let spMap: Map<string, StateProcessThread> = new Map<
            string,
            StateProcessThread
        >();
        let sptMap: Map<string, StateProcessThread> = new Map<
            string,
            StateProcessThread
        >();
        source.map((d) => {
            if (!(d.end_ts < leftNs || d.start_ts > rightNs)) {
                ...
                if (spMap.has(d.state + '_' + d.processId)) {
                    ...
                    obj2.id =
                        (d.state == 'R+' ? 'RP' : d.state) + '_' + d.processId;
                    obj2.pid = d.state == 'R+' ? 'RP' : d.state;
                    obj2.title =
                        (d.process == null || d.process == ''
                            ? 'Process'
                            : d.process) +
                        '(' +
                        d.processId +
                ...
                if (
                    sptMap.has(d.state + '_' + d.processId + '_' + d.threadId)
                ) {
                    let obj3 = sptMap.get(
                        d.state + '_' + d.processId + '_' + d.threadId
                    );
                    obj3!.count++;
                    obj3!.wallDuration += d.dur;
                    obj3!.avgDuration = (
                        obj3!.wallDuration / obj3!.count
                    ).toFixed(2);
    ...
    queryDataByDB(val: SelectionParam | any) {
        getStatesProcessThreadDataByRange(val.leftNs, val.rightNs).then(
            (result) => {
