            ...
            if (item.length != null && item.length > 0) {
                for (const systemNetworkSummary of item) {
                    if (systemNetworkSummary.startTime == 0) {
                        ...
                        systemNetworkSummary.startTimeStr =
                            Utils.getTimeStampHMS(
                                systemNetworkSummary.startTime
                            );
                    }
                    systemNetworkSummary.durationStr = Utils.getDurString(
                        systemNetworkSummary.duration
                    );
                    systemNetworkSummary.dataReceivedStr =
                        Utils.getBinaryByteWithUnit(
                            systemNetworkSummary.dataReceived
                        );
                    systemNetworkSummary.dataReceivedSecStr =
                        Utils.getBinaryByteWithUnit(
                            systemNetworkSummary.dataReceivedSec
                        );
                    systemNetworkSummary.dataSendStr =
                        Utils.getBinaryByteWithUnit(
                            systemNetworkSummary.dataSend
                        );
                    systemNetworkSummary.dataSendSecStr =
                        Utils.getBinaryByteWithUnit(
                            systemNetworkSummary.dataSendSec
                        );
                    systemNetworkSummary.packetsInStr =
                        ColorUtils.formatNumberComma(
                            systemNetworkSummary.packetsIn
                        );
                    systemNetworkSummary.packetsInSecStr =
                        systemNetworkSummary.packetsInSec.toFixed(2);
                    systemNetworkSummary.packetsOutStr =
                        ColorUtils.formatNumberComma(
                            systemNetworkSummary.packetsOut
                        );
                    systemNetworkSummary.packetsOutSecStr =
                        systemNetworkSummary.packetsOutSec.toFixed(2);
                }
                this.source = item;
...
<lit-table id="tb-network-ability" style="height: auto">
    ...
    sortByColumn(detail: any) {
        ...
        function compare(property, sort, type) {
            return function (a: SystemNetworkSummary, b: SystemNetworkSummary) {
                if (type === 'number') {
                    ...
                    return sort === 2
                        ? b.dataReceived - a.dataReceived
                        : a.dataReceived - b.dataReceived;
                } else if (type === 'dataReceivedSecStr') {
                    return sort === 2
                        ? b.dataReceivedSec - a.dataReceivedSec
                        : a.dataReceivedSec - b.dataReceivedSec;
                } else if (type === 'dataSendStr') {
                    return sort === 2
                        ? b.dataSend - a.dataSend
                        : a.dataSend - b.dataSend;
                } else if (type === 'dataSendSecStr') {
                    return sort === 2
                        ? b.dataSendSec - a.dataSendSec
                        : a.dataSendSec - b.dataSendSec;
                } else if (type === 'packetsInStr') {
                    return sort === 2
                        ? b.packetsIn - a.packetsIn
                        : a.packetsIn - b.packetsIn;
                } else if (type === 'packetsInSecStr') {
                    return sort === 2
                        ? b.packetsInSec - a.packetsInSec
                        : a.packetsInSec - b.packetsInSec;
                } else if (type === 'packetsOutStr') {
                    return sort === 2
                        ? b.packetsOut - a.packetsOut
                        : a.packetsOut - b.packetsOut;
                } else if (type === 'packetsOutSecStr') {
                    return sort === 2
                        ? b.packetsOutSec - a.packetsOutSec
                        : a.packetsOutSec - b.packetsOutSec;
                } else {
                    // @ts-ignore
        ...
        if (detail.key === 'startTime') {
            ...
            this.source.sort(
                compare(detail.key, detail.sort, 'dataReceivedStr')
            );
        } else if (detail.key === 'dataReceivedSecStr') {
            this.source.sort(
                compare(detail.key, detail.sort, 'dataReceivedSecStr')
            );
            ...
            this.source.sort(
                compare(detail.key, detail.sort, 'dataSendSecStr')
            );
            ...
            this.source.sort(
                compare(detail.key, detail.sort, 'packetsInSecStr')
            );
            ...
            this.source.sort(
                compare(detail.key, detail.sort, 'packetsOutSecStr')
            );
        } else {
