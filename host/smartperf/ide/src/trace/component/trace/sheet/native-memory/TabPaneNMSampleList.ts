...
export class TabPaneNMSampleList extends BaseElement {
    ...
    static native_type: Array<string> = [
        ...
        if (merageSample.startTs > leftTime) {
            ...
            childSample.timestamp =
                SpNativeMemoryChart.REAL_TIME_DIF == 0
                    ...
                    : formatRealDateMs(
                          merageSample.startTs +
                              SpNativeMemoryChart.REAL_TIME_DIF
                      );
            ...
            if (result.length > 0) {
                ...
                let leftTime =
                    TabPaneNMSampleList.tableMarkData.length == 1
                        ? 0
                        : TabPaneNMSampleList.tableMarkData[
                              TabPaneNMSampleList.tableMarkData.length - 2
                          ].startTs;
                result.forEach((item) => {
                    ...
                    if (nameGroup[item.eventType] != undefined) {
                        if (item.subType == null) {
                            this.merageSampleData(
                                leftTime,
                                data.startTs,
                                nameGroup[item.eventType][0],
                                item
                            );
                        } else {
                            let filter = nameGroup[item.eventType].filter(
                                (type: any) => {
                ...
                if (this.sampleTypesList.length > 0) {
                    let sampleTypesListElement =
                        this.sampleTypesList[this.sampleTypesList.length - 1];
                    sampleTypesListElement.forEach(
        ...
        Object.keys(nameGroup).forEach((key) => {
            ...
            if (nameGroup[key].length > 0) {
                nameGroup[key].forEach((child: any) => {
                    ...
                    childSample.totalGrowth = Utils.getByteWithUnit(
                        child.total
                    );
    ...
    static prepChild(
        currentSample: NativeHookSamplerInfo,
        rootSample: NativeHookSamplerInfo
    ) {
    ...
    setRightTableData(sample: NativeHookSamplerInfo) {
        ...
        this.startWorker(args, (results: any[]) => {
            ...
            if (results.length > 0) {
                ...
                thread.title = `${sample.threadName ?? ''}【${
                    sample.threadId
                }】`;
                thread.type = -1;
                source.push(thread);
    ...
    initElements(): void {
        TabPaneNMSampleList.tbl =
            this.shadowRoot?.querySelector<LitTable>('#tb-native-sample');
        TabPaneNMSampleList.tbl!.addEventListener('row-click', (evt: any) => {
            // @ts-ignore
            this.setRightTableData(evt.detail.data);
        });
        TabPaneNMSampleList.tblData =
            this.shadowRoot?.querySelector<LitTable>('#tb-native-data');
        TabPaneNMSampleList.filter =
            this.shadowRoot?.querySelector<TabPaneFilter>('#filter');
        this.shadowRoot
            ?.querySelector<TabPaneFilter>('#filter')!
            .setSelectList(TabPaneNMSampleList.native_type, null);
        this.shadowRoot
            ?.querySelector<TabPaneFilter>('#filter')!
        ...
        TabPaneNMSampleList.filter!.firstSelect =
            TabPaneNMSampleList.filterSelect;
    }
    ...
    filterAllList() {
        TabPaneNMSampleList.source.forEach((rootSample) => {
            ...
            if (TabPaneNMSampleList.filterSelect == '0') {
                ...
                rootSample.totalGrowth = Utils.getByteWithUnit(
                    rootSample.total
                );
                ...
                if (rootSample.tempList.length > 1) {
                    ...
                    rootSample.growth = Utils.getByteWithUnit(
                        rootSample.heapSize
                    );
