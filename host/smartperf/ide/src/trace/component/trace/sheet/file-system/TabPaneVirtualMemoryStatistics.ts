...
export class TabPaneVirtualMemoryStatistics extends BaseElement {
    ...
    initElements(): void {
        ...
        this.filter!.getStatisticsTypeData((type) => {
            ...
            this.tbl!.shadowRoot!.querySelector(
                'div > div.thead > div > div:nth-child(1) > label'
            )!.textContent =
                type == 'operation'
        ...
        result.forEach((item, idx) => {
            if (childMap.has(item[firstLevel] + '_' + item[secondLevel])) {
                ...
                obj1.children.push(
                    this.getInitData(
                        item,
                        firstLevel == 'type' ? 'tname' : 'type',
                        firstLevel == 'type' ? 'tid' : null
                    )
                );
            } else {
        ...
        for (let ks of fatherMap.keys()) {
            ...
            for (let kst of childMap.keys()) {
                if (kst.startsWith(ks + '_')) {
                    ...
                    data.path[firstLevel == 'type' ? 'type' : 'pid'] =
                        node[firstLevel == 'type' ? 'type' : 'pid'];
                    data.path[firstLevel == 'type' ? 'pid' : 'tid'] =
                        data[firstLevel == 'type' ? 'pid' : 'tid'];
                    data.children.forEach((e: any) => {
                        ...
                        e.path[firstLevel == 'type' ? 'type' : 'pid'] =
                            node[firstLevel == 'type' ? 'type' : 'pid'];
                        e.path[firstLevel == 'type' ? 'pid' : 'tid'] =
                            data[firstLevel == 'type' ? 'pid' : 'tid'];
                        e.path[firstLevel == 'type' ? 'tid' : 'type'] =
                            e[firstLevel == 'type' ? 'tid' : 'type'];
                    });
