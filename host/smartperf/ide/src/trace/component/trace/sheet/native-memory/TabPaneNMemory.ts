...
export class TabPaneNMemory extends BaseElement {
    ...
    fromStastics(val: SelectionParam | any) {
        ...
        let typeIndexOf = this.native_type.indexOf(
            val.statisticsSelectData.memoryTap
        );
        if (
            this.statsticsSelection.indexOf(val.statisticsSelectData) == -1 &&
            typeIndexOf == -1
        ) {
            ...
            let index = this.statsticsSelection.findIndex(
                (mt) => mt.memoryTap == val.statisticsSelectData.memoryTap
            );
            if (index != -1) {
    ...
    initElements(): void {
        ...
        this.tbl =
            this.shadowRoot?.querySelector<LitTable>('#tb-native-memory');
        this.tblData =
                ...
                if (data.mark) {
                        ...
                        new CustomEvent('triangle-flag', {
                            detail: {
                                ...
                                timeCallback: (t: any) => {
                                    ...
                                    let filterTemp = this.source.filter(
                                        (tempItem) => {
                                            if (
                                                minTs == 0 ||
                                                (tempItem.startTs - t != 0 &&
                                                    Math.abs(
                                                        tempItem.startTs - t
                                                    ) < minTs)
                                            ) {
                                    ...
                                    if (filterTemp.length > 0) {
                                        ...
                                        let currentSelection =
                                            this.queryResult.filter((item) => {
                                                return (
                                                    item.startTs ==
                                                    this.rowSelectData.startTs
                                                );
                                            });
                                        if (currentSelection.length > 0) {
                                            currentSelection[0].isSelected =
                                                true;
                                        }
                                        TabPaneNMSampleList.addSampleData(
                                            this.rowSelectData
                                        );
                                        this.tbl!.scrollToData(
                                            this.rowSelectData
                                        );
                                    }
                                },
                    ...
                    if (this.responseTypes.length > thirdIndex) {
                        this.filterResponseType =
                            this.responseTypes[thirdIndex].key == undefined
                                ? -1
                                : this.responseTypes[thirdIndex].key;
                    }
