...
export class TabPanePowerDetails extends BaseElement {
    ...
    queryDataByDB(val: SelectionParam | any) {
        getTabPowerDetailsData(val.leftNs - val.leftNs, val.rightNs).then(
            (items) => {
                ...
                items.forEach((item) => {
                    ...
                    if (item.appKey.toLocaleLowerCase() === 'appname') {
                        ...
                        currentAppIndex = item.eventValue
                            .split(',')
                            .indexOf(SpHiSysEventChart.app_name!);
                        tsMax = 0;
                        ...
                        if (set.has(item.appKey)) {
                            ...
                            powerDatum[item.appKey.toLocaleLowerCase()] +=
                                parseInt(
                                    item.eventValue.split(',')[currentAppIndex]
                                );
                        }
                    }
                });
    ...
    setEnergyItems(
        ...
        let ratio =
            (powerData[energyName].getTotalEnergy(isSimpleEnergy) * 100) /
            totalEnergy;
        if (totalEnergy == 0) {
