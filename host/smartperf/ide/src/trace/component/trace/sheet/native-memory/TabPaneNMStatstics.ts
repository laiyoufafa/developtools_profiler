...
export class TabPaneNMStatstics extends BaseElement {
    ...
    setMallocTableData(
        ...
        result.map((malloc) => {
            ...
            if (malloc.eventType == 'AllocEvent') {
                data.memoryTap =
                    'Malloc ' + Utils.getByteWithUnit(malloc.heapSize);
            } else {
                data.memoryTap =
                    'Mmap ' + Utils.getByteWithUnit(malloc.heapSize);
            }
    ...
    initResponseTypeList(val: SelectionParam | any) {
        ...
        queryNativeHookResponseTypes(val.leftNs, val.rightNs, types).then(
            (res) => {
                procedurePool.submitWithName(
                    'logic1',
                    'native-memory-init-responseType',
                    res,
                    undefined,
                    () => {}
                );
    ...
    setMemoryTypeData(
        ...
        if (anonymous != null) {
            ...
            anonymous.existingString = Utils.getByteWithUnit(
                anonymous.existing
            );
            anonymous.totalBytesString = Utils.getByteWithUnit(
                anonymous.totalBytes
            );
            anonymous.freeByteString = Utils.getByteWithUnit(
                anonymous.totalBytes - anonymous.existing
            );
            anonymous.existingValue = [
                anonymous.existing,
                anonymous.totalBytes,
                this.allMax,
            ];
            ...
            if (column == 'existingString') {
                this.tbl!.recycleDataSource = arr.sort((a, b) => {
                    return sort == 1
                        ...
                        : b.existing - a.existing;
                });
            } else if (column == 'allocCount') {
                this.tbl!.recycleDataSource = arr.sort((a, b) => {
                    return sort == 1
                        ? a.allocCount - b.allocCount
                        : b.allocCount - a.allocCount;
                });
                ...
                this.tbl!.recycleDataSource = arr.sort((a, b) => {
                    return sort == 1
                        ? a.totalBytes -
                              a.existing -
                              (b.totalBytes - b.existing)
                        : b.totalBytes -
                              b.existing -
                              (a.totalBytes - a.existing);
                });
            } else if (column == 'freeCount') {
                this.tbl!.recycleDataSource = arr.sort((a, b) => {
                    return sort == 1
                        ? a.freeCount - b.freeCount
