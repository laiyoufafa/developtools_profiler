...
export class SportRuler extends Graph {
    ...
    private readonly notifyHandler:
        ...
        | undefined;
    private readonly flagClickHandler:
        | ((flag: Flag | undefined | null) => void)
        | undefined;
    ...
    draw(): void {
        ...
        this.flagList.forEach((flagObj: Flag, b) => {
            if (
                flagObj.time >= this.range.startNS &&
                flagObj.time <= this.range.endNS
            ) {
                flagObj.x = Math.round(
                    (this.rulerW * (flagObj.time - this.range.startNS)) /
        ...
        if (
            ...
            typeof this.invertedTriangleTime != undefined
        ) {
            this.drawInvertedTriangle(
                this.invertedTriangleTime,
                document.querySelector<SpApplication>('sp-application')!.dark
                    ? '#FFFFFF'
                    : '#000000'
            );
        }
        ...
        if (time != null && typeof time != undefined) {
            ...
            if (type == 'triangle') {
                ...
                if (i !== -1) {
                    if (triangle !== -1) {
                        this.flagList[i].type == ''
                            ? this.flagList.splice(triangle, 1)
                            : '';
                    }
                    this.flagList.forEach((it) => (it.selected = false));
                ...
                if (i != -1) {
                    ...
                    let triangle = this.flagList.findIndex(
                        (it) => it.type == 'triangle'
                    );
                    if (triangle !== -1) {
    ...
    drawInvertedTriangle(time: number, color: string = '#000000') {
        if (time != null && typeof time != undefined) {
            let x = Math.round(
                (this.rulerW * (time - this.range.startNS)) /
                    (this.range.endNS - this.range.startNS)
            );
            this.c.beginPath();
    ...
    drawSlicesMark(
        ...
        if (
            ...
            let startX = Math.round(
                (this.rulerW * (startTime - this.range.startNS)) /
                    (this.range.endNS - this.range.startNS)
            );
            let endX = Math.round(
                (this.rulerW * (endTime - this.range.startNS)) /
                    (this.range.endNS - this.range.startNS)
            );
            this.c.beginPath();
            if (document.querySelector<SpApplication>('sp-application')!.dark) {
    ...
    mouseUp(ev: MouseEvent) {
        if (this.edgeDetection(ev)) {
            ...
            if (findFlag) {
                ...
                let flagAtRulerTime = Math.round(
                    ((this.range.endNS - this.range.startNS) * x) / this.rulerW
                );
                if (
                    flagAtRulerTime > 0 &&
                    this.range.startNS + flagAtRulerTime < this.range.endNS
                ) {
                    this.flagList.push(
                        new Flag(
                            x,
                            ...
                            18,
                            18,
                            flagAtRulerTime + this.range.startNS,
                            randomRgbColor(),
                            true
                        )
                    );
            ...
            this.flagClickHandler &&
                this.flagClickHandler(this.flagList.find((it) => it.selected));
        }
    }
