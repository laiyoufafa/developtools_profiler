...
export class Top20ThreadCpuUsage extends BaseElement {
    ...
    initElements(): void {
        ...
        this.cpuSetting!.cpuSetListener = ()=>{
            ...
            (this.shadowRoot!.querySelector("#small")! as any).style.display = CheckCpuSetting.small_cores.length > 0 ? 'grid' : 'none';
            (this.shadowRoot!.querySelector("#mid")! as any).style.display = CheckCpuSetting.mid_cores.length > 0 ? 'grid' : 'none';
            (this.shadowRoot!.querySelector("#big")! as any).style.display = CheckCpuSetting.big_cores.length > 0 ? 'grid' : 'none';
            this.queryData();
        }
    ...
    getArrayDataBySize(type:string,arr:Array<any>){
        ...
        for (let obj of arr) {
            if(type === 'total'){
                data.push({pid:obj.pid, pName:obj.pName, tid:obj.tid, tName:obj.tName, total:obj.big, size:"big core",timeStr:obj.bigTimeStr})
                data.push({pid:obj.pid, pName:obj.pName, tid:obj.tid, tName:obj.tName, total:obj.mid, size:"middle core",timeStr:obj.midTimeStr})
                data.push({pid:obj.pid, pName:obj.pName, tid:obj.tid, tName:obj.tName, total:obj.small, size:"small core",timeStr:obj.smallTimeStr})
            }else{
                data.push({pid:obj.pid, pName:obj.pName, tid:obj.tid, tName:obj.tName, total:obj[type],timeStr:obj[`${type}TimeStr`]})
            }
        }
        return data;
    }
