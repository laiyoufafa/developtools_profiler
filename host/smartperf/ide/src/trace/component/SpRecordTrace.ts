...
import {
    ...
    MenuGroup,
    MenuItem,
} from '../../base-ui/menu/LitMainMenu.js';
...
export class SpRecordTrace extends BaseElement {
                            ...
                            (result) => {
                                if (result) {
                                    HdcDeviceManager.shellResultAsString(
                                        CmdConstant.CMD_GET_VERSION,
                                        false
                                    ).then((version) => {
                                        let deviceVersionItem =
                                            SpRecordTrace.supportVersions.filter(
                                                (item) =>
                                                    version.indexOf(item) != -1
                                            );
                                        if (deviceVersionItem.length > 0) {
                                            SpRecordTrace.selectVersion =
                                                deviceVersionItem[0];
                                            this.setDeviceVersionSelect(
                                                SpRecordTrace.selectVersion
                                            );
                                        } else {
                                            SpRecordTrace.selectVersion =
                                                SpRecordTrace.supportVersions[0];
                                            this.setDeviceVersionSelect(
                                                SpRecordTrace.selectVersion
                                            );
                                        }
                                        this.traceCommand!.hdcCommon =
                                            PluginConvertUtils.createHdcCmd(
                                                PluginConvertUtils.BeanToCmdTxt(
                                                    this.makeRequest(),
                                                    false
                                                ),
    ...
    freshMenuDisable(disable: boolean) {
        let mainMenu = this.sp!.shadowRoot?.querySelector(
            '#main-menu'
        ) as LitMainMenu;
    ...
    initElements(): void {
        ...
        this.deviceSelect = this.shadowRoot?.querySelector(
            '#device-select'
        ) as HTMLSelectElement;
        this.deviceVersion = this.shadowRoot?.querySelector(
            '#device-version'
        ) as HTMLSelectElement;
        ...
        this.deviceVersion.onchange = () => {
            let versionItem =
                this.deviceVersion!.options[this.deviceVersion!.selectedIndex];
            SpRecordTrace.selectVersion =
                versionItem.getAttribute('device-version');
        ...
        this.disconnectButton =
            this.shadowRoot?.querySelector<LitButton>('.disconnect');
        ...
        this.progressEL = this.sp.shadowRoot?.querySelector(
            '.progress'
        ) as LitProgressBar;
        this.litSearch = this.sp.shadowRoot?.querySelector(
            '#lit-search'
        ) as LitSearch;
        if (
            this.deviceSelect!.options &&
            this.deviceSelect!.options.length > 0
        ) {
            this.disconnectButton!.hidden = false;
            this.recordButton!.hidden = false;
        } else {
        ...
        this.stopButtonEl =
            this.traceCommand!.shadowRoot?.querySelector<HTMLButtonElement>(
                '#stop-button'
            );
        this.stopButtonEl!.addEventListener('click', (ev) => {
            this.stopRecordListener();
        });
        this.menuGroup = this.shadowRoot?.querySelector(
            '#menu-group'
        ) as LitMainMenuGroup;
        this.appContent = this.shadowRoot?.querySelector(
            '#app-content'
        ) as HTMLElement;
    ...
    stopRecordListener() {
        if (this.vs) {
            let cmd = Cmd.formatString(CmdConstant.CMS_HDC_STOP, [
                SpRecordTrace.serialNumber,
            ]);
            ...
            HdcDeviceManager.connect(selectedOption.value).then((result) => {
                if (result) {
                    ...
                    try {
                        ...
                        HdcDeviceManager.stopHiprofiler(
                            CmdConstant.CMS_STOP,
                            true
                        ).then((result) => {});
                    } catch (exception) {
                        log(exception);
    ...
    usbConnectionListener(event: USBConnectionEvent) {
        if (event.isTrusted) {
            ...
            option.selected = true;
            option.textContent = usbDevice!.serialNumber
                ? usbDevice!.serialNumber.replace(/"/g, '')
                : 'hdc Device';
            this.deviceSelect!.appendChild(option);
            SpRecordTrace.serialNumber = option.value;
        }
    ...
    usbDisConnectionListener(event: USBConnectionEvent) {
        ...
        for (
            let index = 0;
            index < this.deviceSelect!.children.length;
            index++
        ) {
            let option = this.deviceSelect!.children[
                index
            ] as HTMLOptionElement;
            if (option.value == disConnectDevice.serialNumber) {
                ...
                if (SpRecordTrace.serialNumber == optValue) {
                    ...
                    if (options.length > 0) {
                        let selectedOpt =
                            options[this.deviceSelect!.selectedIndex];
    ...
    recordButtonListener() {
        ...
        if (this.vs) {
            ...
            Cmd.execHdcCmd(
                Cmd.formatString(CmdConstant.CMS_HDC_STOP, [
                    SpRecordTrace.serialNumber,
                ]),
                (stopRes: string) => {
                    let cmd = Cmd.formatString(CmdConstant.CMD_MOUNT_DEVICES, [
                        SpRecordTrace.serialNumber,
                    ]);
                    Cmd.execHdcCmd(cmd, (res: string) => {
                        this.sp!.search = true;
                        ...
                        this.litSearch!.clear();
                        this.litSearch!.setPercent(
                            'tracing  ' +
                                this.recordSetting!.maxDur * 1000 +
                                'ms',
                            -1
                        );
                        ...
                        Cmd.execHdcTraceCmd(
                            ...
                            (traceResult: string) => {
                                if (
                                    ...
                                    let cmd = Cmd.formatString(
                                        CmdConstant.CMD_FIEL_RECV_DEVICES,
                                        [
                                            SpRecordTrace.serialNumber,
                                            this.recordSetting!.output,
                                        ]
                                    ...
                                    Cmd.execFileRecv(
                                        ...
                                        (rt: ArrayBuffer) => {
                                            this.litSearch!.setPercent(
                                                'downloading Hitrace file ',
                                                101
                                            );
                                            let fileName =
                                                this.recordSetting!.output.substring(
                                                    this.recordSetting!.output.lastIndexOf(
                                                        '/'
                                                    ) + 1
                                                );
                                            ...
                                            if (
                                                fileHandler &&
                                                !SpRecordTrace.stopRecord
                                            ) {
                                                this.freshMenuDisable(false);
                                                this.freshConfigMenuDisable(
                                                    false
                                                );
                                    ...
                                    this.litSearch!.setPercent(
                                        'tracing htrace failed, please check your config ',
                                        -2
                                    );
                                    this.traceCommand!.show = false;
                                    this.freshMenuDisable(false);
                    ...
                    try {
                        HdcDeviceManager.stopHiprofiler(
                            ...
                            HdcDeviceManager.shellResultAsString(
                                ...
                                this.freshConfigMenuDisable(true);
                                HdcDeviceManager.shellResultAsString(
                                    CmdConstant.CMD_SHELL + traceCommandStr,
                                    false
                                ).then((traceResult) => {
                                    ...
                                    if (re == 0) {
                                        ...
                                        HdcDeviceManager.shellResultAsString(
                                            CmdConstant.CMD_TRACE_FILE_SIZE +
                                                this.recordSetting!.output,
                                            false
                                        ).then((traceFileSize) => {
                                            if (
                                                traceFileSize.indexOf(
                                                    'No such'
                                                ) != -1
                                            ) {
                                                this.litSearch!.setPercent(
                                                ...
                                                Number(traceFileSize) <=
                                                SpRecordTrace.MaxFileSize
                                            ) {
                                                HdcDeviceManager.fileRecv(
                                                    this.recordSetting!.output,
                                                    (perNumber: number) => {
                                                        this.litSearch!.setPercent(
                                                            'downloading Hitrace file ',
    ...
    private isSuccess(traceResult: string): number {
        if (
            traceResult.indexOf('CreateSession FAIL') != -1 ||
            traceResult.indexOf('failed') != -1
        ) {
            return 1;
    ...
    private makeRequest = () => {
        ...
        if (this.record_template) {
            ...
            if (
                this.spVmTracker!.startSamp &&
                this.spVmTracker!.process != ''
            ) {
                hasSamps = true;
            }
            if (
            ...
            if (this.spAllocations!.appProcess != '') {
                request.pluginConfigs.push(
                    this.createNativePluginConfig(reportingFrequency)
                );
            }
            if (this.spRecordPerf!.startSamp) {
                request.pluginConfigs.push(
                    this.createHiperConfig(reportingFrequency)
                );
            }
            ...
            if (
                this.spSdkConfig!.startSamp &&
                this.spSdkConfig!.getPlugName() != ''
            ) {
         ...
         <div class="header">
           <div style="display: flex;margin-bottom: 24px;margin-left:20px;">
             <span class="target">Target Platform:</span>
              ...
              border_radius="16px" back='#0A59F7' opacity="0.6" border="0 solid">Record</lit-button>
              </div>
             </div>
    ...
    private createHiperConfig(reportingFrequency: number) {
        ...
        if (
            perfConfig?.process &&
            !perfConfig?.process.includes('ALL') &&
            perfConfig?.process.length > 0
        ) {
        ...
        if (
            perfConfig?.cpu &&
            !perfConfig?.cpu.includes('ALL') &&
            perfConfig?.cpu.length > 0
        ) {
        ...
        if (
            perfConfig?.eventList &&
            !perfConfig?.eventList.includes('NONE') &&
            perfConfig?.eventList.length > 0
        ) {
    ...
    private createSystemConfig() {
        ...
        if (
            systemConfig?.process &&
            !systemConfig?.process.includes('ALL') &&
            systemConfig?.process.length > 0
        ) {
    ...
    private createNativePluginConfig(reportingFrequency: number) {
        ...
        if (appProcess.indexOf('(') != -1) {
            processId = appProcess.slice(
                appProcess.lastIndexOf('(') + 1,
                appProcess.lastIndexOf(')')
            );
        } else {
        ...
        if (Number(SpRecordTrace.selectVersion) >= 4.0) {
            ...
            nativeConfig.recordAccurately =
                this.spAllocations!.record_accurately;
            nativeConfig.offlineSymbolization =
                this.spAllocations!.offline_symbolization;
            if (this.spAllocations!.record_statistics) {
                nativeConfig.statisticsInterval =
                    this.spAllocations!.statistics_interval;
            }
        }
    ...
    private createMemoryPluginConfig(
        ...
        if (hasMonitorMemory) {
            SpRecordTrace.ABALITY_MEM_INFO.forEach((va) => {
                memoryconfig.sysMeminfoCounters.push(
                    sysMeminfoTypeFromJSON(va)
                );
            });
        }
        this.probesConfig!.memoryConfig.forEach((value) => {
            ...
            if (value.indexOf('Virtual memory stats') != -1) {
                SpRecordTrace.VMEM_INFO.forEach((me) => {
                    memoryconfig.sysVmeminfoCounters.push(
                        sysVMeminfoTypeFromJSON(me)
                    );
                });
    ...
    private createHtracePluginConfig() {
        let tracePluginConfig: TracePluginConfig = {
            ftraceEvents: this.createTraceEvents(
                this.probesConfig!.traceConfig
            ),
            hitraceCategories: [],
            hitraceApps: [],
            bufferSizeKb: 2048,
        ...
        if (this.probesConfig!.traceEvents.length > 0) {
            tracePluginConfig.hitraceCategories =
                this.probesConfig!.traceEvents;
        }
    ...
    freshConfigMenuDisable(disable: boolean) {
        let querySelectors =
            this.shadowRoot?.querySelectorAll<LitMainMenuItem>(
                'lit-main-menu-item'
            );
    ...
    freshMenuItemsStatus(currentValue: string) {
        let litMainMenuGroup = this.shadowRoot?.querySelector<LitMainMenuGroup>(
            'lit-main-menu-group'
        );
        let litMainMenuItemNodeListOf =
            litMainMenuGroup!.querySelectorAll<LitMainMenuItem>(
                'lit-main-menu-item'
            );
