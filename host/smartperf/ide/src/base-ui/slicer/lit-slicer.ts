...
export class LitSlicer extends HTMLElement {
    ...
    set direction(val: any) {
        if (val.startsWith('h')) {
            ...
            this.shadowRoot!.querySelector('div')!.style.flexDirection =
                'column';
        }
    }
        ...
        if (parentDirection.startsWith('h')) {
            ...
            this.line!.onmousedown = (e) => {
                ...
                document.onmousemove = (e1) => {
                    if (this.draging) {
                        if (
                            preWidth + e1.pageX - preX >=
                                this.normalWidth - this.rangeLeft &&
                            preWidth + e1.pageX - preX <=
                                this.normalWidth + this.rangeRight
                        ) {
                            previousElementSibling!.style.width =
                                preWidth + e1.pageX - preX + 'px';
                        }
                    }
                };
            ...
            this.line!.onmousedown = (e) => {
                ...
                document.onmousemove = (e1) => {
                    if (this.draging) {
                        previousElementSibling.style.height =
                            preHeight + e1.pageY - preY + 'px';
                    }
                };
                document.onmouseleave = (e2) => {
                    this.draging = false;
