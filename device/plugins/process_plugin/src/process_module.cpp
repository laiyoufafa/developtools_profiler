...
std::unique_ptr<ProcessDataPlugin> g_plugin = nullptr;
std::mutex g_taskMutex;
} // namespace

static int ProcessDataPluginSessionStart(const uint8_t* configData, uint32_t configSize)
{
    std::lock_guard<std::mutex> guard(g_taskMutex);
    g_plugin = std::make_unique<ProcessDataPlugin>();
    return g_plugin->Start(configData, configSize);
}

static int ProcessPluginReportResult(uint8_t* bufferData, uint32_t bufferSize)
{
    std::lock_guard<std::mutex> guard(g_taskMutex);
    if (g_plugin == nullptr) {
        return -1;
...
static int ProcessPluginSessionStop()
{
    std::lock_guard<std::mutex> guard(g_taskMutex);
    g_plugin->Stop();
    g_plugin = nullptr;
...
static int ProcessRegisterWriterStruct(const WriterStruct* writer)
{
    return 0;
}
