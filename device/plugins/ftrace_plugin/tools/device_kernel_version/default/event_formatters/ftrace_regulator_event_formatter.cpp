...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        int len = snprintf(buffer, sizeof(buffer), "regulator_set_voltage: name=%s (%d-%d)", msg.name().c_str(),
                           (int)msg.min(), (int)msg.max());
...
REGISTER_FTRACE_EVENT_FORMATTER(
    ...
    [](const ForStandard::FtraceEvent& event) -> std::string {
        ...
        char buffer[BUFFER_SIZE];
        int len = snprintf(buffer, sizeof(buffer), "regulator_set_voltage_complete: name=%s, val=%u",
