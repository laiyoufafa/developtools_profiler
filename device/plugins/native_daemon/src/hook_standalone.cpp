...
void ReadShareMemory(uint64_t duration, const std::string& performance_filename)
{
    ...
    while (true) {
        bool ret = g_shareMemoryBlock->TakeData([&](const int8_t data[], uint32_t size) -> bool {
            ...
            if (stackSize > 0) {
                g_runtimeInstance->UnwindStack(u64regs, stackData, stackSize, rawData->pid, rawData->tid, callsFrames,
            ...
            writeFrames(rawData->type, rawData->ts, rawData->addr, rawData->mallocSize, callsFrames);
            return true;
        });
        if (!ret) {
            break;
...
bool StartHook(HookData& hookData)
{
    ...
    HILOG_INFO(LOG_CORE, "hookConfig = %" PRIu64 ",  hookservice smbFd = %d, eventFd = %d\n",
