...
#include "profilergetcapabilities_fuzzer.h"
...
namespace OHOS {
bool ProfilerGetCapabilitiesFuzzTest(const uint8_t* data, size_t size)
{
    ...
    profilerService->GetCapabilities(serverContext.get(), &request, response.get());
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ProfilerGetCapabilitiesFuzzTest(data, size);
    return 0;
}
