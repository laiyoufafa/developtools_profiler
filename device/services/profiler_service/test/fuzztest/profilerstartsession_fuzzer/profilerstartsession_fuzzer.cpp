...
#include "profilerstartsession_fuzzer.h"
...
namespace OHOS {
bool ProfilerStartSessionFuzzTest(const uint8_t* data, size_t size)
{
    ...
    profilerService->StartSession(serverContext.get(), &request, response.get());
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ProfilerStartSessionFuzzTest(data, size);
    return 0;
}
