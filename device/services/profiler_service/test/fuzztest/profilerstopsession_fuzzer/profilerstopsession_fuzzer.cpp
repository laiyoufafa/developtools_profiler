...
namespace OHOS {
bool ProfilerStopSessionFuzzTest(const uint8_t* data, size_t size)
{
    ...
    profilerService->StopSession(serverContext.get(), &request, response.get());
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ProfilerStopSessionFuzzTest(data, size);
    return 0;
}
