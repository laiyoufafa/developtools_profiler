...
#include "profilerdestroysession_fuzzer.h"
...
namespace OHOS {
bool ProfilerDestroySessionFuzzTest(const uint8_t* data, size_t size)
{
    ...
    profilerService->DestroySession(serverContext.get(), &request, response.get());
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ProfilerDestroySessionFuzzTest(data, size);
    return 0;
}
