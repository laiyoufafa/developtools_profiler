# trace_streamer工具说明
trace_streamer工具可以将系统离线trace文件解析并转为db，此工具支持基于文本的trace和基于proto的trace。
关于trace解析工具的使用说明：
...
```
"//developtools/profiler/trace_analyzer/src:trace_streamer_source"
```
...
#### 3.2 windows系统和Mac系统
```
使用QtCreator打开trace_streamer.pro文件，配置默认编译器，执行编译命令（ctrl + b)即可。
具体方法可参考《如何独立编译trace_streamer.md》
```